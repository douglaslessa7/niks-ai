...
Use esta imagem para mandíbula, região pré-auricular, lateral e asa do nariz, e relevo da bochecha sob luz rasante.`
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Chaves que a UI (`skin-result`) consome e que o schema SEMPRE exige. Servem
    // de teste de completude da resposta: relatório sem elas aparece pela metade na
    // tela (região sem texto, "Oleosidade —", sem "Por onde começar"), sem nenhum
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      const response = await fetch(openaiUrl, {
        ...
        body: JSON.stringify({
          ...
          // inclui os tokens de raciocínio. Com 4096, ~3% das análises voltavam
          // CORTADAS no meio do JSON (paravam em `acne` e todo o resto do relatório
          // sumia, sem erro nenhum). Medido no banco em ago/2026 — e acontecia nas
          // DUAS funções, não só nesta: a `analyze-skin` do onboarding foi subida
      ...
      if (!choice) {
        ...
        console.error(`[analyze-skin-app] tentativa ${attempt}: ${lastError}`)
        if (attempt < 3) continue
        break
      }

      // Observabilidade: é por aqui que se descobre truncamento sem precisar do
      // relato de uma usuária. `finish_reason: 'length'` = bateu no teto acima.
      console.log(`[analyze-skin-app] tentativa ${attempt} · multi=${isMulti} · finish_reason=${choice.finish_reason} · usage=${JSON.stringify(data.usage ?? {})}`)
    ...
    if (!result) {
      console.error(`[analyze-skin-app] falhou nas 3 tentativas. Último erro: ${lastError}`)
      return new Response(
    ...
    if (bestMissing.length > 0) {
      ...
      console.error(`[analyze-skin-app] devolvendo resposta INCOMPLETA — ausentes: ${bestMissing.join(', ')}`)
    }
