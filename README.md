...
#### Quem vê: `pending` + `seen`, e por que UM flag não resolve

**Usuária nova é quem chegou à home vindo do fim do onboarding** — não é "quem ainda não viu". Um flag único (`seen`, default `false`) faria o tutorial aparecer para **toda usuária que atualizasse o app**, exatamente o que não pode acontecer. Por isso são dois:

- **`homeTutorialPending`** — **armado em `(onboarding)/notifications.tsx`**, dentro de `navigateToApp()`. Essa é a **única** rota de usuária nova até a home; as outras (`login`, `paywall-soft` de reengajamento, `index` com sessão) são de conta já existente. Quem loga numa conta antiga — **inclusive em aparelho novo** — nunca passa por essa linha e nunca arma.
- **`homeTutorialSeen`** — trava permanente, gravada ao concluir a última parada (`finishHomeTutorial` faz as duas coisas: desarma e tranca).

Os dois são **persistidos** (`partialize`): `pending` porque o app pode morrer entre o onboarding e a home, `seen` porque "uma vez na vida" tem de sobreviver a fechar o app. ⚠️ **Efeito colateral saudável do desenho:** o `signOut` limpa o store persistido (`persist.clearStorage()`), o que zera os dois — e, como `pending` também volta a `false`, quem loga de novo **não** vê o tutorial; só quem passa por um onboarding novo.

⚠️ **App morto no meio do tutorial → ele volta na próxima entrada na home** (o `pending` ainda está armado e `seen` ainda não). É o comportamento desejado: quem não viu até o fim não viu.
...
Perfil → seção **"Desenvolvimento"** → **"Rever tutorial da home"** (`replayHomeTutorial()` rearma `pending` e limpa `seen`, e leva para a home). A seção é `{__DEV__ && …}`: **some sozinha em qualquer build de release, não há nada para reverter antes de subir**. Sem isso, testar exigiria reinstalar o app — mesma dor do `scanTutorialSeen` e do consentimento de IA.
