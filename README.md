...
>
> ⚠️ **ANDROID é o OPOSTO: para REMOVER permissão, o `--clean` é OBRIGATÓRIO (set/2026).** Todos os avisos acima são de **iOS** (capability de In-App Purchase, pasta `ios/NIKS/`, Podfile) — **não valem para `-p android`**. O `withPermissions` do Expo **só concatena** (`new Set(existing.concat(...))`, em `@expo/config-plugins/build/android/Permissions.js`): um `prebuild -p android` **sem** `--clean` lê o `AndroidManifest.xml` que já existe e **nunca remove nada**. Ao tirar `FOREGROUND_SERVICE` do `app.json`, o prebuild não-clean apagou o `<service>` do expo-audio (esse tem remoção explícita) mas **deixou as duas permissões no manifesto** — quem auditasse concluiria "removido" olhando um arquivo que mente. `npx expo prebuild -p android --clean` é **seguro**: rodado 3× aqui, com `ios/` conferida arquivo a arquivo (34 arquivos, mesmo mtime e tamanho) e intacta nas três. A pasta `android/` é gitignorada e 100% gerada — o EAS Build a recria do zero a cada build.
>
> ⚠️ **As opções dos plugins de mídia no `app.json` são LOAD-BEARING — não "simplificar" para string solta.** `["expo-audio", { "enableBackgroundPlayback": false, "recordAudioAndroid": false }]`, `["expo-camera", { "recordAudioAndroid": false }]` e `"granularPermissions": []` no `expo-media-library` existem para o app **não** declarar permissão que não usa. Os padrões dos plugins fazem o contrário: `enableBackgroundPlayback` vem **ligado** (→ `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MEDIA_PLAYBACK` e o serviço `AudioControlsService`), `recordAudioAndroid` também (→ `RECORD_AUDIO`), e o `expo-media-library` pede `['photo','video','audio']` (→ as três `READ_MEDIA_*`). Voltar para `"expo-audio"` reintroduz tudo **em silêncio** — o app funciona igual e só o Google reclama. `RECORD_AUDIO` e `SYSTEM_ALERT_WINDOW` precisam **também** de `android.blockedPermissions`: os manifestos de `expo-audio`/`expo-camera` declaram `RECORD_AUDIO` e a fusão do Gradle o traria de volta.
...
| `generate-skin-preview` | ✅ | `{ image: string }` — base64 da foto do rosto (com ou sem prefixo `data:image/...`) | `{ preview_url: string }` — **URL assinada (2h)** da preview no bucket **privado** `skin-previews`. ⚠️ **Até set/2026 era `getPublicUrl` num bucket PÚBLICO, com nome previsível (`preview_{timestamp}.jpg`): um rosto gerado por IA acessível por URL sem autenticação.** Corrigido com `createSignedUrl` — e a chave da resposta continua `preview_url` **de propósito**, foi o que permitiu corrigir só na Edge Function, sem build nova (o app faz `setSkinPreviewUrl(data.preview_url)` e joga num `<Image>`). ⚠️ **O arquivo não tem `user_id` no caminho e a URL não é gravada em tabela nenhuma** (`skinPreviewUrl` vive só em memória, fora do `partialize`) — logo **nenhum SQL consegue apagar a preview de um usuário específico**; quem limpa é a `cleanup-skin-previews` por idade. Chama **OpenAI Images Edit API** (`gpt-image-2`, `quality: 'medium'`) — única função do projeto que usa OpenAI (não Gemini). Faz upload do resultado em `skin-previews`. Requer secret `OPENAI_API_KEY`. ⚠️ **Antitravamento:** a chamada à OpenAI tem **timeout de 90s** (`AbortController` → responde `504`) e a geração usa `quality: 'medium'` — antes, sem `quality` (padrão pesado) nem timeout, uma OpenAI congestionada deixava a função pendurada por minutos e a tela final travava no spinner "para sempre". **Só o loading do ONBOARDING (`app/(scan)/loading.tsx`) dispara essa geração** — chama com **timeout de 100s + 3 tentativas**; a `plan-preview` tem **fallback em 45s** (cai na própria foto do usuário se a preview não chegar). ⚠️ **`loading-dentro-app.tsx` (scan feito DENTRO do app) NUNCA deve chamar `generate-skin-preview`** — a preview "antes/depois" é peça exclusiva do onboarding/paywall (decisão do usuário). |
| `cleanup-skin-previews` | ⏳ deploy pendente | Header `x-cleanup-secret: <CLEANUP_SECRET>` (o pg_cron não tem sessão → `verify_jwt = false` no `config.toml`, deploy `--no-verify-jwt`). Body vazio. | `{ ok, cutoff, expired, removed, failed }` — apaga do bucket `skin-previews` tudo com **mais de 2h**. ⚠️ **Usa `.remove()` da API de Storage, NÃO `delete from storage.objects`:** o SQL some com o registro mas pode deixar os **bytes órfãos** no backend — inaceitável para imagem de rosto. Agendada por `pg_cron` de hora em hora (`0 * * * *`), com o segredo no **Vault** (a definição de um job é legível em `cron.job`; segredo ali seria o mesmo que não ter segredo). A preview é usada UMA vez, na `plan-preview` do onboarding, e nunca mais — por isso expirar por idade resolve sem precisar saber de quem é cada arquivo. Requer secret `CLEANUP_SECRET`. |
| `delete-account` | ⏳ deploy pendente | Header `Authorization: Bearer <session_token>` (JWT verificado localmente via `_shared/jwt.ts`). **Sem body — o `user_id` vem do JWT, NUNCA do corpo**, senão qualquer usuário autenticado apagaria a conta de qualquer outro. | `{ ok, removed }` — remove os arquivos do usuário em `scans`, `product-scans` e `coach-images` (todos no padrão `{user_id}/…`) com **`.remove()` da API de Storage**, e **só depois** chama `auth.admin.deleteUser`. **A ordem é deliberada:** uma falha na limpeza aborta tudo e a conta fica intacta para nova tentativa; ao contrário, a conta sumiria e os arquivos ficariam órfãos sem nenhum id para reencontrá-los. `skin-previews` fica de fora (não tem `user_id` no caminho — ver `generate-skin-preview`). Chamada por `deleteAccount` em `hooks/useAuth.ts`. **⚠️ A RPC `delete_user` do banco NÃO pode ser removida** enquanto houver base em build antiga: é ela que essas versões chamam, e "Apagar conta" é exigência da Apple e do Google. Ela vive **no banco, fora do repositório**, e depois desta mudança **não existe mais nenhuma chamada a ela no código** — é invisível para quem só lê o repo, e apagá-la quebra a exclusão de conta de todo mundo que ainda não atualizou. |
...
> 1. **Sair do resultado do scan de pele feito no app** — botão voltar de `(app)/skin-result.tsx` (o resultado do onboarding é outra tela, `(scan)/results.tsx`, e **não** dispara).
