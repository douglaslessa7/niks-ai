...
- **DELETE em produção: SÓ por `id` explícito.** Nunca apagar por `user_id`, `created_at` ou qualquer filtro amplo — nem em conta de teste, nem via service role. Liste as linhas primeiro, colete os `id` e apague `?id=in.(…)`. Vale também para arquivos do Storage (apagar pelos `image_path` das linhas já identificadas). ⚠️ **Incidente real (set/2026):** uma limpeza de 5 scans de teste rodada com `?user_id=eq.<uid>` apagou **21** linhas de `product_scans` e 21 arquivos do bucket `product-scans` — sem desfazer. O filtro por id é a única proteção.

- **NativeWind (`className`)** para estilo — `StyleSheet.create()` só em último caso absoluto
  - **Exceção — telas de perguntas do onboarding (identidade "Novo design app NIKS"):** Todas as telas da jornada de perguntas (`birthday` até `social-proof`, incluindo as rotas condicionais `pregnancy`/`skincare-routine-detail`/`allergies-detail`) usam **inline styles + tokens locais** — sem NativeWind, sem `Colors` constants. ⚠️ **MIGRADAS (jul/2026) da identidade antiga (Playfair serif itálico + teal `#1D3A44` + coral `#FB7B6B`) para a NOVA (Nunito + rosa `#FF9D9D` + ink)** — não reintroduzir o design antigo. Tokens no topo de cada arquivo — **os NOMES são legados, os VALORES são os novos**: `CREAM='#FFFFFF'` (fundo branco, sem LinearGradient), `DEEP='#121212'` (ink/títulos), `DEEP_SOFT='#515151'`, `DEEP_HAIR='rgba(18,18,18,0.10)'`, `CORAL='#FF9D9D'` (rosa da marca), `CORAL_DEEP='#F2808E'` (rosa mais fundo — eyebrows). Fontes **Nunito** (`@expo-google-fonts/nunito`, aliases `fXBold/fBold/fSemi/fReg`) — **sem Playfair**. **Referência canônica: `birthday.tsx`** (espelhar ao criar/editar). Estrutura padrão (layout inalterado): **QHeader** (voltar 40×40 + barra de progresso — `TOTAL=13`, steps 1–13: `birthday=1`…`social-proof=12`, `scan-prep=13`), **QTitleBlock** (eyebrow `CORAL_DEEP` uppercase + título **Nunito ExtraBold** `DEEP` com **destaque em Nunito ExtraBold `#FF9D9D`** — antes Playfair Italic coral — + subtítulo `DEEP_SOFT`), **OptionCards** (pill `borderRadius:100`, borda CORAL + glow), **PrimaryButton** (`height:60`, `borderRadius:100`, bg CORAL). Não usar `QuizLayout`, `LinearGradient` nem `Colors`. ⚠️ **`scan-prep`, `loading`, `results`, `plan-preview` também foram migrados, mas NÃO seguem o molde de perguntas:** `loading` virou o **círculo de porcentagem** copiado da tela de dentro do app (ver "Fluxo de scan"); `results` (onboarding) e `skin-result` (app) usam **hex inline** e agora estão idênticas na identidade nova (coral→`#FF9D9D`, teal→`#1A1A1A`, títulos Georgia serif→sans bold, beges→neutro `#E3E3E6`; verde/âmbar/vermelho **semânticos** mantidos); `plan-preview` tem layout próprio (cards antes/depois, "78%/68%").
  - **Exceção — auth e utilitárias (migradas jul/2026 para a identidade nova):** `login` (sign in / botão "Entrar"), `signup`, `nome` (→ `components/onboarding/NameCapture.tsx`, **compartilhado** com o guard de nome do `(app)/_layout.tsx`) e `notifications` seguem o **mesmo molde de `birthday.tsx`** (Nunito + `#FF9D9D` + ink, fundo branco, sem `Colors`/`LinearGradient` coral, sem Playfair). ⚠️ O botão **"Entrar com Apple"** continua **preto** (diretriz da Apple — só a fonte mudou). `paywall-soft` é só o gateway do Superwall (sem UI própria).
...
> **Skeleton no cold start.** No 1º render de um cold start `homeData` é `null` por um instante (sessão + leitura do disco + rede) — antes a home mostrava os **placeholders vazios** (score "—", barras vazias) por ~3s, parecendo quebrada. Agora a `home.tsx` mostra **skeletons** (`components/Skeleton.tsx` — bloco cinza pulsante) no score, na foto e nos números das métricas enquanto `homeData == null && state !== 'error'`. ⚠️ **Skeleton ≠ estado "nunca escaneou":** quando a busca resolve, `homeData` vira um objeto **mesmo para quem nunca escaneou** (score `null`) — aí o skeleton some e aparece o placeholder legítimo (rostinho + "—" + "faça seu primeiro scan"). O gate é `homeData == null`, não `skinScore == null`. Em erro na 1ª carga sem cache (`state === 'error'`) cai no placeholder em vez de pulsar pra sempre.
...
| `recomendacao-produtos.tsx` ("Adicionar à minha rotina") | `protocolo:${uid}` |
| `lib/colecao.ts` (qualquer escrita na Coleção) | `colecao:${uid}` |

> ⚠️ **`recomendacoes_produtos` NÃO tem chave de cache — e isso é deliberado.** A tela de Produtos e o mapa de produtos da Rotina (`lib/rotinaProdutos`) leem direto do banco a cada foco, porque as fotos são **URLs assinadas que expiram em 1h** (mesma razão da aba "Escaneados"). Então, ao mudar só o produto de um passo, **não há cache de recomendação para invalidar** — o que garante tela fresca é (a) esperar a Edge Function responder antes de navegar e (b) o refetch no foco. Invalidar `protocolo:${uid}` ali é defensivo: esse cache guarda a **rotina em si** (passos/dicas), que não muda quando só o produto muda.
...
| `cerimFont` / `cerimFontReg` / `cerimSkiaFont` | **Nunito** ExtraBold/Bold (era DM Serif) | **Cerimônia (ritual) da Rotina** — migrada p/ Nunito (jul/2026, seção 6). `cerimSkiaFont = useFont(Nunito_800ExtraBold, 84)` no numeral do orb. |

> ⚠️ **`protocolo.tsx` usa Nunito** (`@expo-google-fonts/nunito`, mesmas fontes da `home`/`recomendacao`) **em toda a tela, inclusive na cerimônia/ritual** (migrada de DM Serif para Nunito em jul/2026 — seção 6). Não usa Playfair nem DM Serif.
...
- `produtos` — **catálogo de produtos reais** lido pela Edge Function `recomendar-produtos`. ⚠️ **NÃO está em `supabase/migrations/`** (foi criada pelo dashboard) — para ver o schema ao vivo, consulte o banco, não os arquivos. **92 produtos** (ago/2026): 43 importados (Coreia 28, Canadá 7, França 4, EUA 3, Japão 1) + 49 brasileiros (Creamy 29, Principia 20). RLS ligada; escrita só via service role. `UNIQUE (nome, marca)`.
  - **Vocabulário controlado — 3 colunas têm CHECK constraint:**
    - `categoria` (11): `limpeza`, `limpeza_oleosa`, `tonico`, `serum`, `hidratante`, `oclusivo`, `protetor_solar` + as 4 adicionadas em ago/2026 para os lotes nacionais: `esfoliante`, `olhos`, `tratamento_local`, `tratamento_noturno`. ⚠️ **O `ALTER` que adicionou as 4 foi rodado direto no banco, sem migration** (a tabela não tem histórico versionado).
    - `disponibilidade`: **só `nacional` | `importacao`** — é ORIGEM, não facilidade de compra. ⚠️ Armadilha real: um lote inteiro foi escrito com `'facil'` e teria falhado por completo.
    - `intensidade`: `suave` | `moderado` | `forte`. Nullable, **mas é lida** pelo ranking (`isLight()`) e vai no prompt da IA — deixar NULL faz o produto perder o bônus de "suave" para pele sensível. Sempre preencher.
  - **Sem CHECK, mas por convenção:** `nivel_fama` = `iconico` | `conhecido` (não é lida por nenhum código). `tipos_pele` = `seca`/`normal`/`mista`/`oleosa`. `concerns` e `alergenos` = os códigos já em uso na tabela (novos valores entram em silêncio e quebram o filtro sem erro).
  - ⚠️ **As 4 categorias novas ainda NÃO são recomendadas.** `categoriaProduto()` em `supabase/functions/recomendar-produtos/index.ts` só devolve as **7 antigas** — hoje "Tratamento" vira `serum` e passos de olhos são omitidos em silêncio. Os **15 produtos** em `esfoliante` (7), `tratamento_noturno` (5) e `olhos` (3) entram no banco mas **nunca aparecem na tela**. São dois consertos independentes: mapear as categorias E ensinar os ativos novos ao `detectTargetActive`.
  - **Imagens:** 77/92 com foto. Os 15 sem foto são exatamente os das categorias órfãs acima — ou seja, **todo produto recomendável hoje tem imagem**. ⚠️ Ao corrigir a `categoriaProduto()`, colher essas 15 fotos ANTES, senão os produtos entram no pool sem imagem.
- `recomendacoes_produtos` — recomendação de produtos reais (catálogo `produtos`) por usuária, gerada **uma vez** pela Edge Function `recomendar-produtos` (UNIQUE em `user_id`). **Sobrescrita apenas na regeneração do 1º scan in-app** (chamada com `regenerate: true`); fora disso, nunca regenerada. RLS: usuária lê só a própria linha; escrita só via service role. Migration: `supabase/migrations/20260708120000_create_recomendacoes_produtos.sql`
...
- `rotina_produto_fixado` — o produto que a USUÁRIA escolheu para um passo ("Adicionar à minha rotina", no detalhe de um recomendado **ou de uma alternativa**). Colunas: `user_id`, `passo_key` (nome do passo por `normStepKey` — **sem período**, porque `recomendar-produtos` deduplica passos por nome), `produto_id` (**sem FK**: `produtos` não tem DDL versionada no repo). UNIQUE `(user_id, passo_key)` → fixar de novo substitui. RLS com escrita do cliente. ⚠️ **A escolha dela ganha da IA e da Coleção** — ver a ordem de resolução em "Feature: Minha Coleção". Migration: `20260922130000_create_rotina_produto_fixado.sql`
- `colecao_produtos` — **Minha Coleção**: os produtos que a usuária TEM EM CASA. Colunas: `user_id`, `product_scan_id` (scan de origem, `on delete set null`), `image_path` (foto dela no bucket `product-scans`), `produto_nome`/`produto_marca`/`categoria`/`ativos_detectados` (denormalizados da análise), `compatibilidade` (0–100, **NULL = não identificado**, ou scan antigo anterior ao campo), `veredito`, `status` (`identificado` | `nao_identificado`), `skin_scan_id` (scan contra o qual a compat foi calculada), `resultado` (jsonb — a análise inteira, para o detalhe re-renderizar o `ProductAnalysis` sem re-rodar IA). ⚠️ **RLS com escrita do CLIENTE** (select/insert/update/delete das próprias linhas) — diferente de `product_scans`, que é só service role. Índice único parcial em `(user_id, product_scan_id)` = idempotência do pop-up. ⚠️ **Produto do catálogo NÃO entra aqui** (todo item passou por uma foto dela) — para isso existe `rotina_produto_fixado`. Migration: `20260922120000_create_colecao_produtos.sql` (que também adiciona a policy de **INSERT** em `storage.objects` para o bucket `product-scans` — o app precisa subir a foto do produto não identificado — e a coluna `users.colecao_onboarding_at`). Ver "Feature: Minha Coleção"
...
concerns text[] DEFAULT '{}', objetivo text, -- ⚠️ `objetivo` descontinuado (Sessão 54): coluna preservada, mas o app não lê nem escreve mais nela
...
colecao_onboarding_at timestamptz, -- primeiro fluxo da Minha Coleção: null = esta CONTA nunca passou por ele. Verdade da regra "uma vez por conta" (o flag do store é cache do aparelho e morre no logout). ⚠️ Gravado quando ela RESPONDE o pop-up, não no fim do fluxo — ver "Feature: Minha Coleção". Migration 20260922120000
...
- `produtos` — **PÚBLICO** — fotos do catálogo de produtos. **Tudo na raiz do bucket, sem subpasta.** `imagem_url` guarda a URL pública completa e o prefixo é **único para os 77**: `https://utpljvwmeyeqwrfulbfr.supabase.co/storage/v1/object/public/produtos/<arquivo>`. Nome do arquivo = `marca-identificador.ext`, minúsculo, sem acento, espaços em hífen (ex.: `principia-nc-10.webp`, `creamy-calming-cream.webp`). ⚠️ As 43 fotos antigas são um zoológico — **14 são AVIF apesar da extensão**, e as dimensões variam de 500x500 a 3000x3000; as 34 nacionais foram padronizadas em **webp real 800x800**. Ao subir foto nova, conferir o formato REAL por magic bytes (`file`), não pela extensão.
...
user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
conversation_id uuid NOT NULL,
reason text NOT NULL,                -- justificativa clínica, vinda do bloco [[PROTOCOL_PATCH]] que a NIKS emite
proposed_changes jsonb NOT NULL,     -- { action: add|remove|replace, period: am|pm, step_name, ingredient, instruction, schedule_days, replaces }
status text DEFAULT 'pending',       -- CHECK: pending | approved | rejected | applied | expired
approved_at timestamptz,             -- preenchido quando applied
applied_at timestamptz,              -- preenchido quando applied
created_at timestamptz DEFAULT now()
```

> ⚠️ **RLS:** `coach_protocol_suggestions` tem RLS ligada com a policy `coach_protocol_suggestions_select_own` (só `select` das próprias linhas; migration `20260731170000`). Criação e aprovação são **só por service role** (Edge Functions); o cliente apenas LÊ, para renderizar o card.

### Coach — alteração de protocolo (v2)

A NIKS nunca altera o protocolo direto: **propõe** e a usuária **aprova**. A versão antiga aprovava por texto (um `gpt-4.1-mini` classificava a mensagem em "sim"/"não") — isso lia um pedido novo como aprovação e corrompeu protocolos em produção. Como é hoje:

**1. Criação — a NIKS emite a estrutura.** Ao propor, ela termina com a frase-gatilho — **literal e invariável** — conforme a ação (`"Posso incluir isso no seu protocolo?"` para add/replace; `"Posso remover isso do seu protocolo?"` para remove) seguida de um bloco `[[PROTOCOL_PATCH]]{...json...}[[/PROTOCOL_PATCH]]`. O servidor **corta o bloco do stream** antes de chegar ao cliente (a usuária nunca vê o marcador; hold-back por overlap trata marcador partido entre chunks). `checkForSuggestion` **não usa mais LLM** — parseia o bloco. **Gate duplo:** a sugestão só nasce com a frase-gatilho no texto visível **E** exatamente um bloco válido; descasamentos são recusados e logados, inclusive o cruzamento frase↔ação (frase de inclusão + bloco `remove` = `phrase-include-block-remove`, o caso grave, logado com o payload). Pendente é **escopada por conversa** e **expira em 24h na leitura** (senão travaria toda proposta nova).

**2. Escrita — camada única `supabase/functions/_shared/protocol-write.ts`** (usada pelo niks-chat E pelo approve endpoint — não há duas cópias). `validateProposal` valida o schema em runtime (o que está fora do contrato NÃO é gravado) — **inclusive recusando `ingredient` com MARCA/nome comercial** (`ingredient-com-marca:<marca>`, via `marcaComercialEm`; ver o callout "Subtítulo do passo" na tela de Rotina). ⚠️ **Este arquivo é `_shared`: mexer nele obriga a redeployar as TRÊS functions que o importam** (`niks-chat`, `approve-coach-protocol-change` e `generate-protocol`) — cada deploy carrega a própria cópia do módulo. `applyProposal`: **trava clínica** (retinoide/AHA nunca em `rotina_am`); **passos protegidos** (limpeza/hidratante/protetor nunca removidos; `replace` neles só troca-de-mesma-categoria e sem ativo forte); **remove/replace por id**, resolvido semanticamente (nunca substring); **dedup** (add de ativo já presente → recusa; a operação certa é `replace`); inserção em **posição clínica** por categoria; `schedule_days` → sufixo `(Ter/Qui/Sab)` no `ingredient`; item do Coach carrega `origem: 'coach'`. **Status:** `applied` = protocolo alterado; `approved` = aprovada mas protocolo intacto (pausa contida OU falha de escrita); `rejected` = usuária recusou; `expired` = caducou. A gravação **captura erro** (não marca `applied` se o update falhar). `pause` está **contido** — não apaga, marca `approved` + log `PAUSE_CONTAINMENT`.

**3. Aprovação — card, não texto.** O app novo envia `supportsProtocolCard: true` no body do niks-chat; com a flag, `checkApprovalIntent` (aprovação por texto) **não roda** — a decisão é o botão do card, que chama `approve-coach-protocol-change`. Sem a flag (build antigo), a aprovação por texto é preservada integralmente. O card lê `coach_protocol_suggestions` (RLS acima) após o `onload`, com **poll curto** (a sugestão nasce no `waitUntil`, que pode terminar depois do onload), disparado só quando a frase-gatilho está no texto visível.

**4. JWT local compartilhado — `supabase/functions/_shared/jwt.ts`.** `verifyJWT` (crypto.subtle; HS256 + ES256/RS256) foi extraído da niks-chat para `_shared`, e o approve endpoint passou a usá-lo (era `auth.getUser()`, causa de 401 falsos silenciosos). Os guardas de entrada do endpoint logam com `APPROVE_ENDPOINT_REJECTED`.

**Prefixos de log (medição, propositalmente separados):** `PROTOCOL_REFUSED` (recusa clínica/de schema da proposta — a métrica do que a NIKS propõe), `APPROVE_ENDPOINT_REJECTED` (guardas de entrada do endpoint), `PROTOCOL_EXPIRED`, `PAUSE_CONTAINMENT`.
...
| `niks-chat` | ⏳ **deploy pendente** (bloco `<ProductsAtHome>`) | Header `Authorization: Bearer <session_token>` (JWT do usuário autenticado — **não** a ANON_KEY). O token é obtido em `sendMessage` chamando **sempre** `supabase.auth.refreshSession()` antes do XHR — não `getSession()`. O `refreshSession()` garante um token fresco independentemente de clock skew ou race conditions internas do cliente Supabase. Se `refreshSession` falhar, usa `getSession()` como fallback. Body: `{ conversationId, message?, images?: Array<{base64: string, mimeType: string}>, clientMessageId?, supportsProtocolCard?: boolean }` — `message` e `images` são ambos opcionais, mas pelo menos um deve estar presente; `supportsProtocolCard` avisa que o cliente sabe renderizar o card (ver "Coach — alteração de protocolo (v2)"). `userId` **não vai no body** — é extraído internamente via verificação local do JWT (`verifyJWT` com `crypto.subtle`, suportando HS256 via `NIKS_JWT_SECRET` e ES256/RS256 via `SUPABASE_JWKS`). | Stream `text/plain; charset=utf-8` — resposta da NIKS em tempo real. Pós-stream via `waitUntil`: **corta o bloco `[[PROTOCOL_PATCH]]` do texto** e salva o restante em `coach_messages`; extrai memórias em `coach_memories`; e cria a sugestão em `coach_protocol_suggestions` a partir do bloco (via `checkForSuggestion`, **sem LLM**). A aprovação por texto (`checkApprovalIntent`) só roda em **build antigo** (sem a flag `supportsProtocolCard`); no app novo, a aprovação é pelo card. Ver "Coach — alteração de protocolo (v2)". ⚠️ **O context pack ganhou `<ProductsAtHome>`** (o que ela tem em casa, da `colecao_produtos`) — a NIKS responde "o que eu faço agora?" com o que ela já tem em mãos. Bloco SEMPRE presente, inclusive afirmando coleção vazia. Ver "Feature: Minha Coleção → NIKS Chat". |
| `approve-coach-protocol-change` | ✅ | Header `Authorization: Bearer <session_token>` (JWT do usuário). Body: `{ suggestion_id, approved: boolean }`. `user_id` **não vai no body** — extraído do JWT, **verificado localmente** via `_shared/jwt.ts` (não `auth.getUser()`); os guardas de entrada logam `APPROVE_ENDPOINT_REJECTED`. | `{ success, action: 'applied' \| 'rejected' \| 'not_applied' \| 'expired', protocol? }` — aplica/rejeita uma proposta pendente pela **camada `_shared/protocol-write.ts`** (a mesma do niks-chat): `validateProposal` + `applyProposal` (add/remove/replace; `pause` é **contido**, não apaga; expira em 24h; captura erro de escrita). `applied` → modifica `rotina_am`/`rotina_pm` em `protocolos` e devolve o protocolo final; refusos viram `approved`/`not_applied`. Ver "Coach — alteração de protocolo (v2)". |
| `generate-skin-preview` | ✅ | `{ image: string }` — base64 da foto do rosto (com ou sem prefixo `data:image/...`) | `{ preview_url: string }` — URL pública da preview no bucket `skin-previews`. Chama **OpenAI Images Edit API** (`gpt-image-2`, `quality: 'medium'`) — única função do projeto que usa OpenAI (não Gemini). Faz upload do resultado em `skin-previews`. Requer secret `OPENAI_API_KEY`. ⚠️ **Antitravamento:** a chamada à OpenAI tem **timeout de 90s** (`AbortController` → responde `504`) e a geração usa `quality: 'medium'` — antes, sem `quality` (padrão pesado) nem timeout, uma OpenAI congestionada deixava a função pendurada por minutos e a tela final travava no spinner "para sempre". **Só o loading do ONBOARDING (`app/(scan)/loading.tsx`) dispara essa geração** — chama com **timeout de 100s + 3 tentativas**; a `plan-preview` tem **fallback em 45s** (cai na própria foto do usuário se a preview não chegar). ⚠️ **`loading-dentro-app.tsx` (scan feito DENTRO do app) NUNCA deve chamar `generate-skin-preview`** — a preview "antes/depois" é peça exclusiva do onboarding/paywall (decisão do usuário). |
| `analisar-produto` | ✅ | Header `Authorization: Bearer <session_token>` (JWT do usuário — **não** a ANON_KEY; verificado **localmente** via `verifyJWT`/`crypto.subtle`, HS256 via `NIKS_JWT_SECRET` e ES256/RS256 via `SUPABASE_JWKS`, igual ao `niks-chat`). Body: `{ images: [{ base64, mimeType }], clientScanId?, productName? }` — `images[0]` = foto do PRODUTO (obrigatória), `images[1]` = ingredientes (opcional); sem imagem → 400. `productName` (opcional, máx. 200 chars) = título da página no fluxo "Compartilhar com o NIKS"; entra como **contexto extra não verificado** na mensagem da usuária, com regras explícitas: o título **só ajuda a identificar `produto.nome`/`produto.marca`** (foto contradiz → vale a foto); **`produto.ativos_detectados` vem SOMENTE do que estiver legível na foto** — nunca inferido pelo título (vazio se nada legível); e o título não aumenta a `confianca`. Motivo: no teste, com o título "Vitamina C… Creamy" a IA listou "ácido L-ascórbico" sem ler ingredientes. **Schema de saída inalterado**, clientes antigos não mandam. Deploy `--no-verify-jwt`. **Padrão body-first:** lê `await req.text()` ANTES de qualquer rede (evita 401 falso com imagem grande). | `{ scan_id, ...análise }` (`application/json`). Visão **`gpt-5.4-mini`** (`max_completion_tokens: 4096`, `json_object`, retry 3× em 500/503 — mesmo formato do `analyze-skin`). Monta context pack XML (perfil `users` **+ `skincare_routine_type`/`_description`**, último `skin_scans.full_result`, `protocolos`, `coach_memories` ativas — lógica adaptada do `niks-chat`). Análise em 2 momentos + veredito estruturado: `{ status, produto{nome,marca,categoria,ativos_detectados}, confianca, o_que_faz, veredito ('pode_usar'\|'com_ressalva'\|'evitaria'), **compatibilidade (inteiro 0–100)**, resumo, explicacao, resultado_esperado_para_voce, decisao_rotina{tipo ('adicionar'\|'substituir'\|'manter_rotina'), passo, periodo, produto_substituivel, justificativa}, avisos[] }`. ⚠️ **`resultado_esperado_geral` foi REMOVIDO** (era redundante com o `o_que_faz`, e os dois cards diziam quase a mesma coisa): o **Momento 1 agora cabe inteiro no `o_que_faz`** (2–3 frases: o que o produto é, como age e o que costuma entregar). O `resultado_esperado_para_voce` (versão personalizada, Momento 2) continua. **Scans antigos ainda têm o campo no jsonb** — a tela simplesmente ignora; **sem migration, sem backfill**. ⚠️ **`veredito` e `decisao_rotina.tipo` são eixos INDEPENDENTES** — `veredito` nunca é `'manter_rotina'` (correção explícita no prompt). ⚠️ **`compatibilidade` é a expressão NUMÉRICA do `veredito` (Camada 1 — quão bem o produto serve pra pele dela), não um voto na Camada 2:** o prompt exige que o número caia SEMPRE na faixa do veredito (evitaria 0–35 · com_ressalva 40–70 · pode_usar 75–100) e que a decisão de rotina seja tomada ignorando o score — compatibilidade alta + `manter_rotina` é combinação normal (produto serve, mas a rotina já cobre). É o número grande da tela de resultado (`product-result`), que define o tema de cor da tela. **Scans ANTIGOS não têm o campo** (`product_scans.resultado` guarda o objeto inteiro, então o campo novo persiste sozinho — sem migration, sem backfill): todo consumo precisa tolerar `undefined` (a tela mostra `—`, nunca "NaN%"). `status: 'ok'` → faz upload da `images[0]` no bucket `product-scans` + insere em `product_scans` (idempotente por `client_scan_id`, com try/catch — a análise volta pro app mesmo se a escrita falhar). `status: 'precisa_foto'` → não persiste. Secrets: reusa `OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `NIKS_JWT_SECRET`/`SUPABASE_JWKS` (nenhum novo). |
| `extrair-imagem-produto` | ⏳ deploy pendente | Header `Authorization: Bearer <session_token>` (JWT verificado localmente via `_shared/jwt.ts`). Body: `{ sourceUrl }`. Deploy `--no-verify-jwt`. **Body-first.** Usada pela feature **"Compartilhar com o NIKS"** (link vindo do share sheet) e chamada ANTES do `analisar-produto`. | `200 { status: 'ok', imageBase64, mimeType, titulo, imageUrl, finalUrl }` ou `200 { status: 'sem_imagem', titulo, motivo: 'pagina_bloqueada'\|'sem_metatag'\|'imagem_invalida' }`; 400 URL inválida; 401. **Sem IA, sem escrita no banco, nenhum secret novo.** Baixa o HTML (UA de iPhone, `pt-BR`), segue redirects **manualmente** (máx. 5, cobre links encurtados; `meta refresh` = 1 hop extra), e busca a imagem nesta ordem: JSON-LD `Product.image` (inclusive em `@graph`) → **Amazon:** `data-a-dynamic-image` da imagem principal, tentando primeiro a **original sem o sufixo de tamanho** (`…_AC_SS288_.jpg` → `….jpg`, ~1000px — a versão mobile só lista miniaturas) → `og:image:secure_url` → `og:image` → `twitter:image` → `link[rel=image_src]`. **Página de verificação/captcha** (URL final `/account-verification`, `/captcha`, `/cdn-cgi/challenge-platform`…; marcadores de captcha no HTML **só contam se a página não trouxe nenhuma imagem** — reCAPTCHA de newsletter aparece em lojas normais e não pode derrubar uma og:image válida) → `motivo: 'pagina_bloqueada'`; **403/429/503** idem. ⚠️ **4xx comuns (404 etc.) → `motivo: 'pagina_indisponivel'`, NÃO `pagina_bloqueada`** — era assim e um link quebrado fazia o app abrir a WebView por 25 s numa página inexistente.
| | | **URL que JÁ É imagem:** se a resposta vier com `content-type: image/*`, a função devolve a foto direto, sem procurar metatags. ⚠️ Não dá para decidir pela extensão — a VTEX serve `/arquivos/ids/158355-1200-auto?width=1200`, sem extensão nenhuma. **Resolução mínima:** os candidatos são baixados **na ordem** e o primeiro com **lado menor ≥ 400px** vence; menores ficam como reserva (teto de 5 tentativas). Antes valia o primeiro que baixasse — e no Shopify o JSON-LD aponta para a variante `_small`, então a Sallve vinha em **100×100**, ilegível para a IA. Variantes conhecidas são normalizadas para a original antes de tentar (Shopify `_small`/`_100x100`, VTEX `ids/123-450-450`, Amazon `_AC_SS288_`). A dimensão sai do **cabeçalho** do arquivo (PNG/JPEG/WebP), sem decodificar. (antes saía `sem_metatag`, escondendo o bloqueio). Título: `og:title` → `twitter:title` → JSON-LD `name` → `<title>`. **Fallback Mercado Livre:** lê DOIS IDs da URL de origem — **anúncio** (`item_id:MLB…`/`wid=MLB…` do botão Compartilhar do app, ou `MLB-123…` no path do `produto.mercadolivre`) → `/items/{id}`; **catálogo** (`/p/MLB…`) → `/products/{id}` como alternativa. ⚠️ **Hoje as duas rotas respondem 403/401 sem token** (set/2026): a página do ML manda para `/gz/account-verification` e a API exige autenticação — ver "Mercado Livre: estado da integração" na feature. Aceita só JPEG/PNG/WebP (AVIF/SVG → próximo candidato), máx. 5 MB; HTML truncado em 2 MB; timeout 8 s por fetch. ⚠️ **Proteção SSRF — não remover:** só http/https, recusa localhost/IPs privados/link-local **em cada hop** (inclusive após `resolveDns`). Logs `extrair-imagem-produto: ok\|sem_imagem host=… motivo=…`. |
| `recomendar-produtos` | ⏳ **deploy pendente** (Minha Coleção) | `{ user_id, scan_id?, regenerate?, preservar_catalogo? }` (fetch direto + `apikey`; deploy `--no-verify-jwt`). A chamada mora em **`lib/recomendarProdutos.ts`** (fonte única — `generateProtocol` e os caminhos da Coleção usam ela). ⚠️ **`preservar_catalogo: true`** quando o motivo da rechamada é a **Minha Coleção**: passos intocados mantêm o produto que já tinham, senão a camada de IA (não determinística) trocaria em silêncio os recomendados de passos sem relação. | `{ recomendacao }` — array de passos com produtos do catálogo `produtos`. ⚠️ **Resolve cada passo na ordem `fixado` → `em_casa` → catálogo**: o produto que ELA fixou (`rotina_produto_fixado`) ganha da Coleção e da IA; o da Coleção ganha do catálogo. Passo resolvido pelos dois primeiros **nem chega à camada de IA** (`fixado` sai com o produto dela como principal; `em_casa` sai com `produtos: []`). O fixado é **revalidado** no pool elegível antes de ser aplicado. ⚠️ **Pontua cada produto recomendado (0–100) numa 2ª chamada, DEPOIS de responder** (`EdgeRuntime.waitUntil`, mesmo padrão do pós-stream do niks-chat): a tela recebe a recomendação assim que ela existe e as notas entram na linha em seguida. Só pontua produto **sem nota** — com `preservar_catalogo`, passo intocado mantém produto E nota, então fixar um produto normalmente não dispara chamada nenhuma. Gates do produto dela: categoria (mesmo `classifyStep` dos passos) · gestante · alérgeno legível · **retinoide/AHA nunca de manhã** · não dobrar família forte no período · ativo-alvo nos passos de Tratamento. Ver "Feature: Minha Coleção". **Auto-guardada (gera UMA vez):** se já existe linha em `recomendacoes_produtos` p/ o user, retorna a existente sem regenerar. **Exceção:** chamada com `regenerate: true` no body (regeneração do 1º scan in-app) **pula o guard e sobrescreve** a linha — as recomendações passam a bater com o protocolo novo (`ignoreDuplicates: !regenerate`). Busca no banco (perfil em `users`, scan em `skin_scans.full_result`, protocolo em `protocolos`). Classifica cada passo com o **`classifyStep` portado verbatim de `protocolo.tsx`** → categoria de produto. Filtro SQL de elegibilidade **generoso** (categoria + `tipos_pele` + `ativo`; gestante → só `seguro_gestante`; concerns só ranqueiam, não filtram) com **corte DURO de alérgeno** (map keyword de `allergy_description` → códigos `alergenos`; na dúvida corta). **GATE DE INGREDIENTE (só passos `classifyStep==='Tratamento'`):** extrai o ativo-alvo do `name`/`ingredient` (`detectTargetActive`) e exige que `ativos_principais` contenha o ativo — **AHA é classe** (mandélico/glicólico/láctico/aha/pha se substituem), **BHA/salicílico e azelaico são exatos**, **bakuchiol nunca vira retinoide** (segurança gestante). Isso corrige o bug "passo de vit C recebia azelaico" (os 4 séruns colapsavam no mesmo pool). Passo de Tratamento gateado sem match → entrada `{ sem_produto: true, motivo }`; **`sem_produto` é EXCLUSIVO de Tratamento gateado**. Passos de olhos/olheiras e `Cuidado` (não reconhecido) → **omitidos em silêncio** (sem mensagem); staples nunca viram `sem_produto`. **Desempate de passos-irmãos** (2+ passos na mesma categoria staple, ex.: "Hidratante Leve" AM vs "Hidratante com Ceramidas" PM): nudge de ranqueamento por intenção do passo (`stepIntent` — preferência, nunca corte) + não repete o mesmo principal entre irmãos se houver alternativa distinta. Camada de IA (`gpt-4.1-mini`, `json_object`) escolhe 1–3 por passo (traz 2–3 quando há alternativas boas e distintas), marca 1 principal, sempre **dentro do conjunto elegível** (validado por id). **Copy nunca vazia:** se a IA não devolve copy, sintetiza de `marca + ativo + ideal_para` (`synthCopy`). Disparada por [lib/generateProtocol.ts](lib/generateProtocol.ts) logo após o insert de `protocolos` (não bloqueante). Requer `OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. |
...
**Modelos de IA:**
- `analyze-skin` (onboarding, 1 foto) e `analyze-skin-app` (app, multi-foto): **`gpt-5.4-mini` (OpenAI)** — `response_format: json_object`, **`max_completion_tokens: 16000`**, retry em HTTP 500/503. Mesmo modelo hoje; a `analyze-skin-app` pode subir para um modelo melhor no futuro sem afetar o onboarding
  - ⚠️ **NÃO baixar o teto de volta para 4096 — foi um bug real, medido (ago/2026).** `max_completion_tokens` é o teto de tokens de SAÍDA e, em modelo de raciocínio, **inclui os tokens de raciocínio**. Com 4096, **~3% das análises voltavam CORTADAS**: o JSON parava logo depois de `acne` e sumiam pigmentação, textura/poros, brilho sebáceo, envelhecimento, `region_insights`, `prioridade_clinica`, `skin_strengths`. **Sem erro nenhum** — a tela só aparecia pela metade ("Oleosidade —", região sem texto, sem "Por onde começar"). Assinatura no banco: `full_result` com **13 chaves em vez de 29–30** (as 9 primeiras do schema + as 4 que a própria função preenche no fim: `skin_type_detected`, `pontos_fortes`, `pontos_fracos`, `disclaimer`) — dá para caçar com `select count(*) from jsonb_object_keys(full_result)`. Atingia as **duas** funções em proporção parecida (não era exclusivo do multi-foto, como pareceu no começo). Subir o teto **não encarece** o caso normal: cobra-se pelo que é escrito, e o relatório completo usa bem menos que 16000.
  - **Rede de segurança nas duas funções:** `REQUIRED_KEYS` + `missingKeys()` conferem a completude da resposta; `finish_reason === 'length'` ou campo obrigatório faltando → **retenta** (dentro das mesmas 3 tentativas). Na última, devolve a resposta **menos incompleta** em vez de erro (score + headline + acne já valem uma tela, e falhar aqui queimaria mais 3 chamadas no retry do cliente) e loga `devolvendo resposta INCOMPLETA`. Cada tentativa loga `finish_reason` + `usage` — é por aí que se vê truncamento sem depender de print de usuária. ⚠️ **Manter a lista `REQUIRED_KEYS` em sincronia entre as duas funções**; só pôr nela campo que o prompt exige SEMPRE (campo condicional viraria retentativa cara à toa).
  - ⚠️ **Campo ausente não pode virar achado clínico.** A tela derivava "poros dilatados" de `textura_poros?.pore_visibility !== 'normal'` — e `undefined !== 'normal'` é **true**, então os relatórios cortados **afirmavam** poros dilatados que ninguém analisou. Corrigido nas duas telas (`skin-result.tsx` e `results.tsx`) exigindo `!= null` antes de comparar. **Ao criar regra derivada de campo da IA, testar sempre o caso "campo não veio"** — o padrão seguro é `?.present` / `?.includes(...)` (undefined → falsy); comparação por desigualdade (`!== 'normal'`) é o caso perigoso.
...
- `niks-chat`: **`gpt-4.1-mini` (OpenAI)** — streaming principal + pós-stream via `waitUntil`: 1 chamada não-streaming (`extractAndSave`, memórias). `checkForSuggestion` **não chama LLM** (parseia o bloco `[[PROTOCOL_PATCH]]` que a NIKS emite); `checkApprovalIntent` (1 chamada) só em build antigo sem card
...
- `analisar-produto`: **`gpt-5.4-mini` (OpenAI)** — visão (imagem do produto + ingredientes), `response_format: json_object`, `max_completion_tokens: 4096` (**nunca `max_tokens`** — família GPT-5 quebra), retry 3× em HTTP 500/503 (mesmo formato do `analyze-skin`). ⚠️ **Mesma combinação (GPT-5 + 4096) que truncava o `analyze-skin`** — mas **aqui não trunca** (conferido ago/2026: 40/40 `product_scans.resultado` com as 13 chaves e o último campo do schema, `avisos`, presente), porque a saída é bem menor. **Se um dia o resultado do produto aparecer pela metade, é aqui que se olha primeiro** — mesmo diagnóstico, mesma correção. `generate-protocol` e `recomendar-produtos` usam `gpt-4.1-mini` e **não** têm essa exposição (sem tokens de raciocínio consumindo o teto).
...
- **JSON mode nas chamadas secundárias do `niks-chat`** (`extractAndSave`, `checkApprovalIntent`): usam `response_format: { type: 'json_object' }` da API OpenAI — garante JSON válido sem preamble (não há `finishReason` para checar). `max_tokens`: 512 para `extractAndSave`; 128 para `checkApprovalIntent`. **`checkForSuggestion` não chama a OpenAI** — parseia o bloco `[[PROTOCOL_PATCH]]` emitido pela NIKS; ainda usa `extractJSON` (delimitadores `{`/`}`) para tolerar espaços em volta do JSON.
...
- **Camada de validação determinística (após a IA responder, ANTES de devolver — nunca deixa sem protocolo):** reusa `detectTargetActive` **e `marcaComercialEm`** de `_shared/protocol-write.ts` (que ganhou o campo **aditivo** `molecule`; Coach inalterado). **Checagem 3** (todos os tipos): `ingredient` com MARCA/nome comercial (`CHECK3_BRAND_IN_INGREDIENT`) — ver o callout "Subtítulo do passo" na tela de Rotina. **Checagem 1** (só `skincare_routine_type` `complement`/`prescribed`): o ativo declarado em `skincare_routine_description` precisa aparecer como **PASSO** — por **molécula** (adapaleno ≠ retinol, embora ambos classe `retinoide`). **Checagem 2** (todos os tipos): dois ativos da mesma classe no mesmo período. Violação → **uma** 2ª chamada corretiva (mesmo modelo/params, timeout 60s); se ainda falhar, grava assim mesmo. Logs `PROTOCOL_VALIDATION` (`CHECK1_MOLECULE_SUBSTITUTED` = caso "adapaleno→retinol", medido à parte). **No prompt**, os bullets de `complement`/`prescribed` e `skincare_routine_description` passaram a exigir o ativo declarado como PASSO + a **regra de família de ativos** (um por família — retinoides/AHAs/BHA/vit C/niacinamida/azelaico — sem dobrar). **(Validação no ar; as 4 edições de prompt: deploy pendente.)**
...
> ⚠️ **REORDENADO (jul/2026): o welcome deixou de ser um carrossel de 5 telas.** Agora `app/index.tsx` mostra **APENAS a tela 1 ("Bem-vinda ao NIKS")**, e "Começar" leva direto para a etapa de **nome** (`(onboarding)/nome`, "Como você quer ser chamada?"), que passou a vir **antes das perguntas**. As telas 2–5 de apresentação foram **movidas para depois da criação de conta** — vivem no carrossel `app/(onboarding)/apresentacao.tsx`, mostrado após o `signup` (dentro do app, pós-paywall). **O link "Já tem conta? Entrar" (FooterLink) fica só na tela 1** (welcome) — não aparece nas telas 2–5, que agora são pós-cadastro. Os 5 slides são a **fonte única** `components/onboarding/welcomeSlides.tsx` (átomos `VideoClip`/`ProgressDots`/`PrimaryButton`/`FooterLink` + `Screen1`…`Screen5`); `index.tsx` importa `Screen1`, `apresentacao.tsx` importa `Screen2`–`Screen5`. `FooterLink` e `ProgressDots` são condicionais (props `onLogin`/`activeDot`).

Textos das 5 telas de apresentação:

| Screen | Texto principal | Onde aparece agora |
|---|---|---|
| Screen 1 | "Bem-vinda ao NIKS" | `app/index.tsx` (entrada, com "Entrar") |
| Screen 2 | "Seu glow up começa pela pele" | `apresentacao.tsx` (pós-signup) |
| Screen 3 | "Sua expert de pele, sempre disponível" | `apresentacao.tsx` (pós-signup) |
| Screen 4 | "Escolhemos os melhores produtos pra sua pele" | `apresentacao.tsx` (pós-signup) |
| Screen 5 | "Esse produto serve pra mim?" | `apresentacao.tsx` (pós-signup, última) |
...
> ⚡ **Vídeos comprimidos + poster (jul/2026) — conserta a demora de 2–3 s pra o vídeo aparecer.** Os `welcome-*.mp4` chegaram **absurdamente pesados** (welcome-1 = 22 MB, welcome-2 = 34 MB, ~91 MB no total, 1080×1920 @ 5–11 Mbps com faixa de áudio inútil). O `expo-video` precisa bufferizar um bocado do arquivo antes do 1º quadro → 2–3 s de **tela branca**. **Conserto:** (1) reencodei os 5 com ffmpeg para **720×1280, H.264 CRF 27, sem áudio, `-movflags +faststart`** → **~8,4 MB no total** (welcome-1 2,1 MB / welcome-2 3,3 MB / 3 744 KB / 4 960 KB / 5 1,3 MB), **qualidade visualmente idêntica** (o mockup aparece só num pedaço da tela, 720p sobra). (2) **Poster**: cada tela agora passa `poster={require('../../assets/welcome-N-poster.jpg')}` (1º quadro, ~60 KB) pro `VideoClip`, que mostra a imagem **na hora** e faz **fade** dela no evento `playingChange` (quando o vídeo realmente começa) → **nunca mais tela branca**; se o vídeo falhar, o poster fica de fallback. ⚠️ **Ao TROCAR um vídeo, gere o poster novo também** (`ffmpeg -i welcome-N.mp4 -frames:v 1 -q:v 3 welcome-N-poster.jpg`) — senão o poster fica de um vídeo antigo. Originais guardados **fora do repo** em `~/Downloads/niks-welcome-originais-backup/` (não versionar — 91 MB; deletável). ffmpeg foi um binário avulso temporário (não ficou instalado).

**Onde o nome é salvo:** a tela de nome roda em `mode="store"` (NameCapture) porque ainda **não há sessão** (vem antes do signup) — só guarda o nome em `pendingName` no store (persistido, mesmo ciclo do `appliedCoupon`). A gravação em `users.nome` acontece no **`signup`**, dentro de `saveToSupabase`. O guard de nome do `(app)/_layout.tsx` (NameCapture em `mode="session"`) segue intacto como rede de segurança para usuárias legadas sem nome.

> ⚠️ **Identidade visual migrada (jul/2026):** as 5 telas foram trazidas para o **"Novo design app NIKS"** — fonte **Nunito** (não mais Playfair Italic), acento **rosa `#FF9D9D`** (era coral `#FB7B6B`), texto **ink `#121212`/`#515151`/`#818181`** (era teal `#1D3A44`). Tokens locais em `components/onboarding/welcomeSlides.tsx` (fonte única dos slides): `INK/INK_SOFT/INK_MUTE/CORAL='#FF9D9D'/WHITE`. **A palavra de destaque virou Nunito ExtraBold rosa** (era serif itálico). ✅ **Vídeos definitivos JÁ no lugar (jul/2026):** os 5 `require` (em `welcomeSlides.tsx`) apontam para **`../../assets/welcome-1.mp4` … `welcome-5.mp4`** (tela 1→5, nesta ordem: Bem-vinda / glow up / expert / produtos / escanear produto). Trocar um vídeo = substituir o `.mp4` correspondente. ⚠️ Nome de arquivo **sem acento/caractere especial** (o Metro falha **em silêncio**). Os arquivos antigos (`welcome-video.mp4.mp4`, `glowup-screen-video.mp4`, `expert-screen-video.mp4`, `alimentacao-screen-video.mp4`, `transicao-screen-video.mp4`) **não são mais referenciados** — podem ser deletados. **Copy atual (jul/2026):** tela 4 = "Escolhemos os **melhores produtos** pra sua pele"; tela 5 = "Esse produto serve **pra mim**?" (era "Em poucas semanas… de um jeito diferente") — a tela 5 agora fala em **escanear um produto** para ver compatibilidade, então o vídeo do espelho (`welcome-5.mp4`) pode não combinar mais com a copy (pendente de decisão do usuário).
...
> ⚠️ **`rate-us.tsx` (`(scan)`) saiu do fluxo de onboarding.** Era o passo entre `social-proof` e `scan-prep` e disparava o popup nativo de avaliação da App Store (`requestAppReview()`) no `useEffect`. A **Apple não permite** pedir avaliação antes de o usuário realmente usar o app, então o gatilho foi desativado: `social-proof` agora navega direto para `scan-prep`/`skin-result`, e o `requestAppReview()` + o import estão comentados na `rate-us.tsx`. **O arquivo continua no projeto** (não foi deletado), mas hoje é **código morto**: a avaliação foi reintroduzida dentro do app **sem** passar por ele (ver abaixo).

> ⭐ **Avaliação (popup nativo da App Store) DENTRO do app — 3 momentos de "dopamina".** A avaliação voltou, mas agora só depois de uma ação concluída que entrega valor. Todos os pontos reusam **`requestAppReview()` de `lib/storeReview.ts`** (`expo-store-review`, com fallback pra abrir a página da App Store se a API não estiver disponível — o mesmo utilitário do botão "Avaliar o app" do perfil). Gatilhos:
> 1. **Sair do resultado do scan de pele feito no app** — botão voltar de `(app)/skin-result.tsx` (o resultado do onboarding é outra tela, `(scan)/results.tsx`, e **não** dispara). ⚠️ **SÓ no voltar:** o CTA flutuante "Seguir para a minha rotina" da mesma tela **não** chama `requestAppReview()`, de propósito — o gatilho é "SAIR do resultado" (ação terminal), enquanto seguir para a rotina é ação de **avanço**, e o pop-up nativo por cima interromperia a usuária no meio da tarefa. Com a chamada num caminho só, "uma vez por visita" vale **por construção**: se os dois disparassem, sair pelo CTA e depois voltar para tocar no voltar dispararia duas vezes na mesma sessão. **Não adicionar a chamada no CTA.**
> 2. **Sair do resultado da análise de produto** — `onClose` do wrapper `(scan)/product-result.tsx`. A chamada fica **só no wrapper**, não em `components/product/ProductAnalysis`, senão a aba "Escaneados" (`recomendacao-produtos.tsx`, que reusa o mesmo componente) dispararia avaliação também. `onRescan` fica de fora de propósito (leva a outro resultado → outro `onClose`).
> 3. **"Voltar ao protocolo"** na tela final da cerimônia (`(app)/protocolo.tsx`, estado `ritualDone`) — é o **mesmo botão para manhã e noite** (`isPM` só muda estilo/texto), então uma chamada cobre os dois modos.
>
> ⚠️ **Comportamento esperado, não bug:** o iOS limita o popup nativo a **~3 vezes por ano por usuário** (silencia as chamadas excedentes) — por isso não há throttle próprio, chamamos em toda ação e o SO controla a frequência. E o popup **não aparece no TestFlight** (só de forma confiável em build de produção; em `__DEV__` costuma aparecer sempre) — a ausência no TestFlight não indica erro.
...
```typescript
{
  ...
  concerns_alignment?: {     // card "Suas preocupações" em results/skin-result (Sessão 55)
    alinhamento: 'confirmado'|'parcial'|'divergente'
    regioes_afetadas: string[]
    mensagem: string         // 2 frases: valida as preocupações (concerns) à luz dos achados + o que o protocolo vai fazer
  }                          // gerado só se a usuária declarou concerns. Substituiu o antigo `goal_alignment` (que era sobre `objetivo`, removido na Sessão 54); scans antigos com `goal_alignment` simplesmente não renderizam o card
...
**Campos de onboarding:** `genero`, `pregnancy_status`, `birthday`, `skin_type`, `concerns[]`, `sun_exposure`, `hydration`, `sleep`, `goal_desire`, `skincare_routine_type`, `skincare_routine_description`, `allergy_type`, `allergy_description` — ⚠️ o campo `objetivo` foi **removido na Sessão 54** (junto com a tela `goal.tsx`)
...
**Botão "Continuar" é flutuante e aparece no meio da rolagem (não no fim — decisão de UX, não bug):** antes o CTA vivia no fim do `<ScrollView>`, então só era descoberto ao rolar tudo. Agora é um **`Animated.View` `position:'absolute'` irmão do ScrollView** (fora dele), fixo na base, que **surge com fade + slide-up** quando o usuário passa pela seção "O que fazer pela sua pele" — o `onLayout` daquela `View` grava `sectionY` e o gatilho é `HERO_HEIGHT + sectionY − viewportH*0.5` (topo da seção alcança o meio da tela). A opacidade/translateY são interpoladas do mesmo `scrollY` (native driver); um `scrollY.addListener` liga `floatActive` só quando visível → `pointerEvents` alterna `'none'`/`'box-none'` pra não capturar toque enquanto invisível. Fallback: resultado **sem** a seção → gatilho vira `maxScroll*0.4` (nunca deixa a usuária sem CTA). `contentContainerStyle.paddingBottom = insets.bottom + 100` reserva espaço pro conteúdo não ficar atrás do botão. **Não devolver o botão pro fluxo do ScrollView "achando que sumiu".**
...
> **📝 Subtítulo do passo (`name` × `ingredient` × produto) — o "texto duplicado" NÃO é bug de render.** O card mostra `title` (= `protocolos.rotina_*.name`) e, abaixo, o subtítulo. `mapStep` **não concatena nada** — `title←name`, `ingredients←ingredient`, e há **um único** ponto de render. A aparência de duplicação vem do **prompt do `generate-protocol`, que manda repetir o TIPO do produto nos dois campos**: `name` = "tipo + benefício, máx. 5 palavras" ("Gel de Limpeza Suave") e `ingredient` = "tipo + ativo + concentração" ("Gel de Limpeza com Ácido Salicílico 2%"). Ou seja, "Gel de Limpeza" nos dois é o comportamento **projetado**.
> **O que mudou:** quando o passo tem produto ESCOLHIDO por ela (Coleção ou fixado), o subtítulo passa a ser **marca + nome do produto** — é o que ela vai pegar na pia. Sem produto, segue o `ingredient` (o ativo é a informação útil enquanto não há produto).
> ✅ **MARCA no `ingredient` — consertado nas duas pontas.** O caso real (confirmado no banco: `rotina_am[0]` com `ingredient` = "Gel de Limpeza **La Roche-Posay Effaclar Gel**") era marca + nome comercial num campo de ATIVO. Marca tem campo próprio (`product_suggestions`, que o app nem renderiza). Conserto:
> - **Detector único** `marcaComercialEm()` em `_shared/protocol-write.ts` — lista explícita de marcas (catálogo + dermocosmético PT-BR) + linhas comerciais ('effaclar', 'anthelios'…), casando por palavra inteira. ⚠️ **É heurística de LISTA, não garantia**: pega o que está na lista. Marca nova escapando nos logs → acrescente lá. ⚠️ **Não pôr na lista palavra que também seja ativo ou tipo de produto** ('cica', 'gel') — recusaria ingrediente legítimo.
> - **`generate-protocol`:** nova **Checagem 3** (`CHECK3_BRAND_IN_INGREDIENT`) na camada de validação determinística, que reusa a retentativa corretiva que já existia — prompt sozinho já tinha falhado em produção. Mais o spec dos campos `ingredient` e `name` proibindo marca explicitamente.
> - **`_shared/protocol-write.ts`:** `validateProposal` **recusa** a proposta do Coach com `ingredient-com-marca:<marca>` (vira `PROTOCOL_REFUSED` no caller). **Recusar e não "limpar"**: tirar "La Roche-Posay" de "Gel de Limpeza La Roche-Posay Effaclar Gel" ainda deixaria "Effaclar Gel", também comercial — e o contrato desta camada é "o que está fora do contrato NÃO é gravado".
> - ⚠️ **Nada disso corrige as linhas JÁ gravadas** em `protocolos` — não houve backfill. Protocolo antigo com marca no `ingredient` continua como está até ser regenerado.

- **Timeline de 5 passos** com trilho tracejado + círculo numerado; cada card **expande inline** ("Como fazer") via `LayoutAnimation` + chevron animado. O **trilho tracejado é desenhado em SVG** (`<Line strokeDasharray="4 4">` com altura medida por `onLayout`, centralizado sob o número) — **não** usar `borderStyle:'dashed'` numa borda de um só lado, que renderiza de forma não-confiável no New Architecture (some ou vira sólido). Dentro do "Como fazer" há um link **"Ver produto recomendado" + seta** (cor `BRAND #FF9D9D`) que **abre direto o detalhe do produto daquele passo**: grava `productDetailStep = { passo: step.title, periodo }` no store e navega — quem casa passo↔produto é a tela de Produtos (ver "Deep-link" lá). Sem match, abre no topo, como antes.
- **Dados reais** (não mais hardcoded): a rotina vem da tabela **`protocolos`** (`select('rotina_am, rotina_pm, dicas')`, mais recente) — **fonte de verdade**, revalidada **a cada foco** via `useCachedQuery` (`staleMs: 0`, stale-while-revalidate: pinta do cache, atualiza em silêncio, sem piscar). O store `protocolResult` (persistido em disco — ver "CACHE DE DADOS") é **fallback do vão**: cobre o onboarding enquanto o insert não landa, ou um insert falho — a tela nunca fica em branco. ⚠️ **Precedência invertida (Bloco 4):** antes o store tinha precedência e a tabela era fallback 1×/5min, o que **escondia alterações feitas no servidor** (aprovação do Coach). Agora a tabela ganha quando tem qualquer período com dado, o store cobre só o vão, e um efeito sincroniza o store a partir da tabela. Cada `ProtocolStep` cru é convertido para a UI por **`mapStep()`**: `title←name`, `ingredients←ingredient`, `how←instruction` (fallback `steps.join`); `category`+`icon` derivados por **`classifyStep()`** (keyword no name+ingredient — **NÃO** usa o hex `color`, que a IA erra). Meta dinâmica: `Passos` = contagem real, `Duração` = `~${n*3} min`, `Foco` = rótulo fixo por período. Estados `loading`/**`generating`**/`ready`/`empty`/`error` (empty → CTA "Escanear minha pele" via `startFaceScan()` do `useFaceScan`; error → "Tentar novamente" — que **rearma** a geração sob demanda; **`generating`** = geração sob demanda em curso, ver callout abaixo). Os passos **crus** (`name/ingredient/steps[]/waitTime`) são mantidos em `amRaw`/`pmRaw` porque a cerimônia precisa deles.
...
**Cerimônia (ritual passo a passo)** — tela imersiva, **migrada para a identidade nova (jul/2026)**. Abre pelo botão **"Iniciar rotina"** (`openRitual` → `setTabBarVisible(false)`; `closeRitual` restaura). ⚠️ **Era "igual à produção" (DM Serif serif + teal `#1D3A44` + coral `#FB7B6B`); foi RECOLORIDA a pedido do usuário:** fonte **Nunito** (inclusive o numeral Skia = `useFont(Nunito_800ExtraBold)`), tinta **ink `#121212`**, acento **rosa `#FF9D9D`**. **Modo DIA recolorido** (gradientes pêssego→rosa suave em `dayGradients`, orb pêssego→rosa, botão teal→ink). **Modo NOITE mantém a estética escura/estrelada DE PROPÓSITO** (fundo escuro + `NightSky` + orb creme/dourado) — só o que era **coral** virou `#FF9D9D` e o **teal** virou ink; o resto do clima noturno fica. Estrutura intacta: **orb Skia** com anéis respiratórios + numeral via `SkiaText`/`useFont`, e **tela de celebração** com entradas escalonadas. ⚠️ **Layout do passo à prova de texto variável** (a instrução do passo é gerada pela IA e muda de tamanho): o bloco **título + instrução + chip é um `ScrollView` dentro de um `<View flex:1>`** (centraliza quando cabe, rola quando é longo) e o **CTA "Concluir este passo" fica EM FLUXO logo abaixo (NÃO é mais `position:absolute`)** — assim a área rolável termina onde o botão começa e o texto **nunca** cola nem passa por baixo do botão. **Não voltar o CTA para absoluto** (era a causa do texto colar/sumir atrás do botão). Overlay `position:absolute zIndex 60` dentro da View raiz (por isso `setTabBarVisible(false)` — a navbar global é sibling em `_layout.tsx`). ⚠️ As fontes **DM Serif foram removidas** do `useFonts` — a cerimônia agora é **100% Nunito** (`cerimFont`/`cerimFontReg` são aliases que apontam para `Nunito_800ExtraBold`/`Nunito_700Bold`). Imports do Skia são aliased (`SkiaCircle/SkiaRect/SkiaRadialGradient`) para não colidir com o `react-native-svg` da tela. `toggleStepCompletion(index)` faz haptic + som + **persiste o progresso** (abaixo). **Streak/gamificação continuam desativados** — a cerimônia marca conclusão de passo mas **não** incrementa `streak_days`.

> **Ajuste fino do modo DIA (manhã) da cerimônia (jul/2026)** — refinamentos a pedido do usuário, **só afetam `!isPM`** (a noite fica intocada):
> - **Passo:** o **número do orb** e o **nome do passo** são **brancos** (`#FFFFFF`), não mais ink. O CTA **"Concluir este passo"** virou **fundo branco + texto/seta ink `#121212`** (⚠️ substitui o "botão teal→ink" descrito acima — no passo do modo dia o botão **não é mais escuro**). O check dentro do botão continua no círculo rosa `#FF9D9D`.
> - ⚠️ **GOTCHA — branco sobre rosa claro é ilegível sem sombra.** O número (Skia) leva um `<SkiaShadow>` e o título leva `textShadow`, ambos numa **cor rosa-escura** (`rgba(180,60,80,...)`). **Não remova** achando que é enfeite: sem a sombra, branco e o rosa claro do fundo têm luminosidade quase igual e o texto **some** (nenhum "tom de branco" resolve — branco já é o mais claro; o contraste vem da sombra escura por baixo). Só existe no modo dia.
> - **Tela de celebração (manhã):** fundo **branco puro `#FFFFFF`** (era o creme rosado `#FFF5F6→#FFECEE`, que lia como "cor errada/sombra"); **orb** num rosa limpo `#FFFFFF→#FFC4C6→#FF9D9D` (removido o coral escuro `#F2808E` da borda + glow reduzido, que davam aspecto de sombra); CTA **"voltar ao protocolo"** agora **rosa `#FF9D9D`** (era ink/preto) com sombra rosa em vez de preta. Coerente com orb-rosa + botão-roso da identidade.
...
> Nota: `protocolo.tsx` lê a tabela `protocolos` como **fonte de verdade** (revalidada no foco), com o store `protocolResult` como **fallback do vão** — precedência **invertida no Bloco 4**; ver "Dados reais" acima. `protocolGenerating` continua sendo escrito por `signup.tsx` e **não** é consumido pela tela.
...
> 🖼️ **Exemplo visual de cada passo (set/2026).** Abaixo de "Foto X de 6" há um **círculo 84pt com a foto-modelo** do passo atual (borda branca + sombra, etiqueta rosa `#FF9D9D` com o nome curto do passo — campo `exampleLabel`: Neutro/Sorrindo/Surpresa/Brava/Esquerda/Direita; começou como "Exemplo" e o usuário trocou), inspirado no app Doji. Título e dica continuam embaixo da moldura; a moldura desceu ~20pt (`paddingTop: 40` em `cameraPlaceholder`) para abrir espaço. Troca com crossfade (`transition` do `expo-image`) + pop de escala a cada passo (sem pop com "Reduzir movimento"); some quando as 6 estão prontas. **Fotos:** `assets/scan-examples/<StepId>.jpg` (360×360, ~25 KB, geradas por IA; o campo `example` de cada item de `STEPS` é a fonte única). Originais em PNG 1,7 MB ficam em `Fotos exemplo pra scan/` na raiz, **fora do bundle**. Trocar uma foto = substituir o .jpg (nome sem acento). ⚠️ **Os exemplos dos PERFIS seguem a SETA e o preview espelhado, não o nome dos arquivos originais:** `perfil_esq` ("vire para a sua esquerda", seta ←) mostra a modelo com o **nariz apontando para a esquerda da tela** — veio do arquivo original "…(perfil direito).png"; `perfil_dir` veio do "…(perfil esquerdo).png". Decisão do usuário; não "corrigir" pelos nomes.

> 🎬 **Animação "foto voando para a pilha" (set/2026).** A cada foto: clarão branco no toque → a foto congela em tela cheia, encolhe num cartão de borda branca e voa em curva (X desacelera, Y acelera) até a pilha do canto inferior esquerdo → a pilha dá um pulinho, o badge salta, `haptics.tap()` (`success()` na 6ª). **Tempos** nas constantes `FLY_SETTLE` (110ms, vira cartão) / `FLY_HOLD` (20ms, pausa) / `FLY_TRAVEL` (360ms, descida) no topo do arquivo — a descida foi aprovada pelo usuário; o "antes da descida" foi encurtado duas vezes a pedido dele. ⚠️ **A foto só entra em `photos` no POUSO** (callback do Reanimated via `runOnJS`) — é isso que faz o contador e a instrução trocarem depois da animação; o mutex `capturingRef` fica preso durante o voo e é solto no pouso. O callback de pouso roda **mesmo se a animação for interrompida** (a foto nunca pode se perder). ⚠️ **O cartão aparece com a foto CRUA (`photo.uri`) e o `downscale` roda em paralelo, sem `await`** — esperar o downscale antes do voo era a maior parte da lentidão. A foto reduzida fica guardada por id de voo (`finalUriRef`), é aguardada no pouso e só ela entra em `photos`; se o downscale falhar, o cartão some e aparece o alerta de erro. O cartão só decola após o `onLoad` da imagem, com timer de segurança de 400ms. ⚠️ **O cartão voador usa `expo-image`, NÃO o `<Image>` do RN:** a foto crua traz a rotação só no EXIF, que o `<Image>` do RN ignora — com ele o cartão aparecia DEITADO e cortado (bug visto no device). A foto reduzida pelo ImageManipulator já sai com a rotação aplicada, por isso a pilha e a revisão seguem com o `<Image>` do RN. O cartão é espelhado (`scaleX:-1`) **só na tela**, para bater com o preview da câmera frontal — a foto salva continua sem `mirror`. As constantes `STACK_*` espelham `styles.stackWrap/stackCard` (destino do voo): **mudou a pilha, mude as duas**. "Reduzir movimento" do iOS ligado → sem animação. No simulador dá pra ver o voo pelo toque na galeria de `__DEV__` (o toque longo, que preenche as 6, não anima).
...
> ⚠️ **ATUALIZADO (Minha Coleção): `loading-dentro-app.tsx` agora chama `lib/colecaoFlow.ts#finalizarPrimeiroFluxo`, NÃO o `regenerateProtocolInApp` direto.** A regeneração continua sendo a mesma e única (com os mesmos guards), só que agora **envelopada**: primeiro a fila de fotos de produto vira a Coleção — contra o scan que acabou de ser salvo —, e só então a rotina é refeita, já enxergando o que ela tem em casa. Sem fila e sem fluxo pendente, o comportamento é idêntico ao descrito abaixo. **Não criar um segundo caminho de regeneração.** Ver "Feature: Minha Coleção".
>
> ⚠️ **O PRIMEIRO scan pelo app regenera a rotina UMA vez; do 2º em diante, NÃO.** `loading-dentro-app.tsx` insere a linha nova em `skin_scans` e — **só no 1º scan in-app** — dispara em **background** (não segura a navegação) `lib/regenerateProtocolInApp.ts` → `generateAndSaveProtocol` (que encadeia `recomendar-produtos` com `regenerate: true`, refrescando também a recomendação). **Marcador:** coluna `users.inapp_protocol_regenerated_at`, gravada **só no sucesso confirmado** — o read-back compara a linha mais recente de `protocolos` **antes × depois** (a do signup sempre existe, então "existe linha" não prova nada); app morto no meio → marcador null → retry no próximo scan (sem dupla, sem estado preso). Ordem de escrita **tabela → store → cache** (a Rotina tem a tabela como fonte de verdade; NÃO usar `onSuccess:setProtocolResult`, que dispara antes do insert). A usuária vê um **card-promessa** na `skin-result` ("Estamos atualizando sua rotina…", presente contínuo). **Do 2º scan em diante o protocolo fica congelado** — a pessoa precisa **testar a rotina por ~30 dias** para medir eficácia; se cada scan reescrevesse, a IA trocaria ativos a cada foto e ninguém saberia o que funcionou. **Motivo do 1º scan regenerar (decisão do usuário):** o `analyze-skin-app` (multi-foto) é o melhor retrato da pele e o momento certo de refazer o protocolo nascido do onboarding (1 foto). **Usuárias legadas** (scan mas nunca protocolo) seguem cobertas pela **geração sob demanda** ao ABRIR a Rotina/Produtos (ver seção 6 e "Tela Recomendação de Produtos"). **(Implementado; deploy pendente — migration `20260801120000` + `recomendar-produtos`.)**
...
| `product_scan_completed` | `hooks/useProductAnalysis.ts` após `analisar-produto` | `origem: 'camera' \| 'share_url' \| 'share_image'`, `status`, `compatibilidade`, `veredito` |
| `product_scan_failed` | `hooks/useProductAnalysis.ts` após esgotar retries; `share-product-loading.tsx` se falhar ao preparar a imagem (`etapa: 'preparar_imagem'`) | `origem`, `error` |
| `product_share_received` | `components/share/ShareIntentBridge.tsx` ao receber conteúdo do share sheet | `origem: 'share_url' \| 'share_image'`, `dominio` |
| `product_share_image_extracted` | `share-product-loading.tsx` quando `extrair-imagem-produto` achou a foto | `dominio` |
| `product_share_no_image` | `share-product-loading.tsx` quando o link não tem foto utilizável | `dominio`, `motivo` |
| `colecao_primeiro_fluxo_perguntado` | `useColecaoOnboarding` ao abrir o pop-up | — |
| `colecao_primeiro_fluxo_respondido` | resposta do pop-up | `tem_produtos` |
| `colecao_primeiro_fluxo_abandonado` | X na `colecao-camera` | `total` (fotos já tiradas) |
| `colecao_fotos_enfileiradas` | sair da `colecao-camera` para o scan de rosto | `total` |
| `colecao_lote_analisado` | `colecaoFlow` após processar a fila | `enfileirados`, `identificados`, `nao_identificados` |
| `colecao_pergunta_respondida` | pop-up pós-scan de produto (`product-result`) | `tem_em_casa`, `compatibilidade`, `veredito` |
| `colecao_item_adicionado` | item entrou na Coleção | `origem: 'scan' \| 'catalogo'` |
| `colecao_item_identificado` | refoto de um item não identificado deu certo | — |
| `colecao_itens_removidos` | remoção na aba Minha Coleção | `total` |
| `colecao_rotina_carregada` | aba Recomendados carregada — **métrica central da feature** | `passos_total`, `passos_com_produto_dela` |
| `rotina_produto_fixado` | "Adicionar à minha rotina" (detalhe de recomendado ou alternativa) | `origem: 'principal' \| 'alternativa'` |
...
Fluxo de scan facial (dentro do app principal):
  botão "Escanear" da home (useFaceScan)
    → conta que NUNCA passou pelo 1º fluxo da Minha Coleção: NÃO abre a câmera —
      vai para /protocolo (Rotina), onde o pop-up "você tem produtos em casa?" começa o fluxo
...
Primeiro fluxo da Minha Coleção (uma vez por CONTA — ver a feature):
  abrir a Rotina pela 1ª vez (ou tocar "Escanear" na home, que leva pra lá)
  → pop-up "Você tem produtos de skincare em casa?"
    → Sim: (scan)/colecao-camera (fotografa todos em sequência, sem análise)
    → Não: nada é enfileirado
  → nos dois casos: scan de pele de DENTRO do app (6 fotos)
  → loading-dentro-app → colecaoFlow#finalizarPrimeiroFluxo (background):
     fila → Coleção → rotina regenerada já com a Coleção
...
Fluxo de produto (dentro do app principal):
  ...
  → 1s depois: pop-up "Você tem esse produto em casa?" → Sim entra na Minha Coleção
    (e passa a ocupar o passo da rotina, se for o mais compatível dele)

Fluxo "Compartilhar com o NIKS" (Share Extension iOS — de fora do app):
  Safari / Mercado Livre / site → Compartilhar → NIKS (extensão NIKSShare)
  → abre niks-ai://dataUrl=… → ShareIntentBridge grava pendingShare no store
  → (app)/_layout (SÓ depois do guard de sessão+assinatura) → share-product-loading
    [link: extrair-imagem-produto → analisar-produto | imagem: analisar-produto]
  → product-result (mesma tela, ProductAnalysis) → salvo em product_scans ("Escaneados")
...
│   │   ├── recomendacao-produtos.tsx ✅ Recomendação por passo, **ligada aos dados reais** (`recomendacoes_produtos`→`produtos`, chips via `lib/concernLabels`, `expo-image`); **TRÊS abas: Recomendados / Minha Coleção / Escaneados**; deep-link da home abre detalhe. Ver "Tela Recomendação de Produtos" e "Feature: Minha Coleção"
...
│   │   ├── perfil.tsx             ✅ novo design (Nunito/branco/rosa) — nome dinâmico, email, notificações, suporte, apagar conta
...
│       ├── camera-multi.tsx       ✅ scan de 6 fotos (4 expressões + 2 perfis) — **DENTRO DO APP** → loading-dentro-app. Monta 2 colagens (ScanCollage) + manda 3 imagens à **analyze-skin-app** (função própria, não a do onboarding). Círculo com foto-exemplo de cada passo + miniaturas empilhadas (com animação da foto voando até a pilha) + tela de revisão com retake. Consentimento de IA. Ver "Scan de pele multi-foto (13b)"
│       ├── colecao-camera.tsx     ✅ câmera em LOTE do primeiro fluxo da Minha Coleção — fotografa os produtos em sequência SEM chamar a IA (só enfileira no store); a análise roda depois, contra o scan novo. Saída única: o scan de rosto. Ver "Feature: Minha Coleção"
...
│       ├── share-product-loading.tsx ✅ loading EXCLUSIVO do "Compartilhar com o NIKS" (anel girando + crossfade rosto↔produto). Aberta só pelo (app)/_layout depois do guard. Lógica via `hooks/useProductAnalysis`
│       ├── food-camera.tsx        ✅
...
│       ├── share-capture.tsx      ✅ réplica da tela "Layout" do Instagram: grade ESCOLHÍVEL, câmera na célula da vez, fundo ao vivo borrado (módulo nativo), sem header/"Continuar"
...
│   │   └── GridIcon.tsx           ✅ glifo de uma grade de colagem, desenhado com Views a partir do próprio `cellRects` (ver "A grade é ESCOLHIDA")
...
├── lib/savedProducts.ts           ⚠️ **APOSENTADO pela Minha Coleção** — sobrou só o `normStepKey` (chave de casamento passo↔produto, usada em 3 lugares). O "produto salvo por passo" em AsyncStorage saiu: quem dá a foto do passo agora é `lib/rotinaProdutos.ts`, a partir do `em_casa` da recomendação
├── lib/colecao.ts                 ✅ **PORTA ÚNICA de escrita da Minha Coleção** (tabela `colecao_produtos`): `listarColecao`, `adicionarDoScan`, `adicionarDoCatalogo`, `adicionarNaoIdentificado`, `identificarItem`, `removerItens` (⚠️ só por id explícito), `entraNaRotina`, `motivoIncompativel`, `VEREDITO_INCOMPATIVEL`. Ver "Feature: Minha Coleção"
├── lib/colecaoFlow.ts             ✅ orquestração do PRIMEIRO FLUXO: `precisaPrimeiroFluxo`/`marcarPrimeiroFluxoFeito` (gate por conta), `processarFilaColecao` (analisa a fila DEPOIS do scan novo) e `finalizarPrimeiroFluxo` (fila → regeneração da rotina → refresh da recomendação). Função de módulo, sobrevive à navegação. `MAX_PRODUTOS_PRIMEIRO_FLUXO = 12`
├── lib/analisarProduto.ts         ✅ a chamada à Edge Function `analisar-produto` (fonte única) — extraída do `useProductAnalysis` quando a análise em LOTE da Coleção passou a precisar dela fora de um componente React
├── lib/recomendarProdutos.ts      ✅ a chamada à `recomendar-produtos` (fonte única). ⚠️ `preservarCatalogo: true` sempre que o motivo for a Coleção — ver a linha da Edge Function
├── lib/rotinaProdutos.ts          ✅ `getProdutosDosPassos()` — foto + marca + nome do produto que ocupa cada passo da Rotina. SÓ passos com produto ESCOLHIDO por ela (`em_casa` da Coleção ou `fixado`); recomendação da IA não entra. URL assinada → nunca cachear. Substituiu o antigo `savedProducts`
├── supabase/functions/_shared/compatibilidade.ts ✅ **FONTE ÚNICA da régua clínica de compatibilidade (Camada 1: veredito + nota 0–100)** — texto VERBATIM do Passo 3 do prompt da `analisar-produto`, que agora o interpola daqui. Importado também pela `recomendar-produtos`. `clampCompatibilidade` garante em código a faixa que o prompt promete (evitaria 0–35 · com_ressalva 40–70 · pode_usar 75–100). ⚠️ Mexer no texto muda a análise por foto TAMBÉM — é esse o ponto
├── lib/rotinaFixados.ts           ✅ `fixarProdutoNoPasso`/`desfixarPasso` — a ESCOLHA dela ("Adicionar à minha rotina"), tabela `rotina_produto_fixado`. ⚠️ Não confundir com a Coleção: fixar = "quero este produto neste passo"; Coleção = "tenho este produto em casa"
├── hooks/useColecaoOnboarding.tsx ✅ o pop-up "você tem produtos de skincare em casa?" — montado na ROTINA, que é o gatilho do primeiro fluxo (o "Escanear" da home redireciona para cá)
├── components/colecao/PerguntaSimNao.tsx ✅ pop-up de pergunta Sim/Não da NIKS — UM componente para as DUAS perguntas da Coleção (primeiro fluxo e pós-scan)
...
├── lib/collageGrid.ts             ✅ **FONTE ÚNICA do layout da colagem** do compartilhável — `COLLAGE_GRIDS` (6 grades, na ordem da bandeja), `getGrid`, `cellRects`, `gridForCount`. Consumido pela captura, pelo preview e pela imagem exportada; a grade viaja no store (`collageGrid`). Ver "Feature: Compartilhar Niks score → A grade é ESCOLHIDA"
├── lib/shareResume.ts             ✅ retomada do share quando a tela de carregamento REMONTA (memória de módulo, janela de 5 min). Sem isso, a 2ª montagem não acha `pendingShare` (já consumido) e volta pra home no meio da análise. Limpa ao abrir o resultado/fechar a tela
├── lib/nativePresentation.ts      ✅ conta seletores NATIVOS abertos (galeria/câmera de `home` e `niks-chat`) — o (app)/_layout espera fechar antes de empilhar o share
├── lib/pageImageScript.ts         ✅ script que lê a foto do produto numa página (WebView). ⚠️ MANTER EM SINCRONIA com `preprocessorInjectJS` do app.json
├── components/share/HiddenPageImageReader.tsx ✅ WebView invisível (quando o backend não acha a foto) — UA do Safari, timeout 12 s, corte de 6 s em tela de verificação, logs `[share-webview]`
├── lib/shareProduct.ts            ✅ "Compartilhar com o NIKS": `normalizeShareIntent`, `firstUrlFromText`, `extractProductImage` (Edge Function `extrair-imagem-produto`), `prepareProductImage`/`…FromBase64` (mesmo downscale da câmera)
├── lib/facePhoto.ts               ✅ `getFacePhotoUrl(uid)` — mesma regra da foto da home (`foto_home_url ?? último skin_scans.foto_url`)
├── lib/sessionToken.ts            ✅ `getAccessToken()` — token de sessão p/ Edge Functions com JWT local (refresh só se <5 min)
...
├── hooks/useProductAnalysis.ts    ✅ **LÓGICA única** do loading de produto (analisar-produto, clientScanId, guard, %, alta demanda, retry, Mixpanel) — usada por `product-loading` e `share-product-loading`
├── components/share/ShareIntentBridge.tsx ✅ copia o conteúdo do share sheet p/ `pendingShare` (nunca navega)
├── app/+native-intent.tsx         ✅ link `dataUrl=` da Share Extension não é rota (cold start → `/`, app aberto → fica)
├── assets/fonts/
...
react-native-webview    ← **13.16.0** (versão do SDK 55). Só a WebView INVISÍVEL do "Compartilhar com o NIKS" (links do Mercado Livre bloqueados no backend). Dependência nativa → rebuild
expo-share-intent       ← **6.1.1** + patch local (`patches/expo-share-intent+6.1.1.patch`). Share Extension iOS `NIKSShare`
...
Uma dica a cada 24h, numa **fila numerada**. A **ordem** da fila é a mesma para todas (sem personalização por perfil), mas cada usuária **começa na dica 1 no seu 1º acesso** e anda no próprio ritmo — a fila é **individual do aparelho**, deslocada pela data em que ela entrou (quem começou ontem está sempre uma casa à frente de quem começou hoje). Substituiu o card de skincare na home (item 6 da estrutura).
...
**Mecânica — `lib/dicas/dicaDoDia.ts`** (AsyncStorage). Duas chaves: `dica_do_dia:index` (contador) e `dica_do_dia:shown_at` (horário, em ms, em que a dica atual foi mostrada). Ao focar a home, `getDicaDoDia()` avança **uma** casa **se — e só se — já passaram ≥ 24h** desde `shown_at`, regravando `shown_at = agora`; dentro das 24h a dica não muda. A fila é **individual do aparelho** (o índice mora só no AsyncStorage): não é uma dica-por-data-de-calendário igual para todo mundo.
- ⚠️ **Avança no máximo UMA casa por chamada**, e só a cada 24h de uso — quem sumir uma semana volta na dica seguinte à que viu, não sete adiante; e quem não viu a 2ª nunca pula para a 3ª. **Corrigido na Sessão 53:** antes a chave era `dica_do_dia:last_day` e a fila andava quando o **dia civil virava** (à meia-noite) — abrir às 23h50 e de novo às 00h10 queimava uma dica em 10 min, não em 24h. Migração suave: usuária antiga (sem `shown_at`) mantém o índice e só reinicia o relógio de 24h a partir de agora.
...
> ⚠️ **Um novo scan NÃO regenera a rotina — isso é intencional, não um bug.** Decisão do usuário: a pessoa precisa **testar a rotina por pelo menos ~30 dias** para medir eficácia. Se cada scan reescrevesse o protocolo, a IA ficaria trocando ativos a cada foto e a usuária nunca saberia o que funcionou. Portanto `loading-dentro-app.tsx` grava o novo `skin_scans` (score + as 6 métricas novas) e **deixa `protocolos` e `recomendacoes_produtos` congelados**. Não "conserte" isso.
>
> ⚠️ **CONSEQUÊNCIA CONHECIDA — corte clínico não se desfaz sozinho. NÃO É BUG.** Os cortes clínicos da recomendação são avaliados **no momento em que ela é gerada**, e a rotina é congelada logo depois. Então um produto cortado por uma condição **transitória** — barreira comprometida, rosácea ativa, gravidez/amamentação — **não volta sozinho** quando um scan posterior mostrar melhora. A pele sarou, o produto continua fora. Isso vale hoje para os cortes que já existem (gestante, alérgeno, e o veredito `evitaria` que tira um produto da Coleção da rotina) e valerá igual para o corte de barreira/rosácea quando ele for implementado.
> **Por que fica assim:** o congelamento é a decisão de produto acima — reavaliar a cada scan é justamente o que ela impede. Corrigir "o produto não voltou" reabriria a rotina a cada foto.
> **As únicas portas de reavaliação** são as que já existem: a regeneração única do 1º scan in-app (`inapp_protocol_regenerated_at`) e uma mudança na Coleção — e esta última **só reavalia o passo tocado**, porque `preservar_catalogo` mantém os outros intactos de propósito.
> ⚠️ **O espelho é o lado mais delicado, e tem a mesma causa:** um produto **aprovado** enquanto a pele estava bem também não é reavaliado se um scan posterior mostrar barreira comprometida. Hoje quem cobre esse caso é o Coach (a NIKS pode propor a alteração, com a usuária aprovando) — não a recomendação. Se um dia isso virar problema clínico de verdade, a saída **não** é descongelar a rotina: é um gatilho explícito de reavaliação quando o scan cruza um limiar de segurança. Fotos via **`expo-image`** (WebP/AVIF). **Chips (tags)** derivadas de `produtos.concerns` traduzidas por **`lib/concernLabels.ts`** (`CONCERN_CODE_TO_LABEL` — fonte única no app, inverso do `CONCERN_LABEL_TO_CODE` da Edge Function `recomendar-produtos` + os códigos derivados do scan; manter em sincronia). **Deep-link — dois canais, ambos abrem o modal de detalhe e zeram o alvo:** `productDetailTarget` = `produto_id` (do "Para você" da home) e `productDetailStep` = `{ passo, periodo }` (do "Ver produto recomendado" de cada passo da **Rotina** — que não conhece o `produto_id`). ⚠️ **O casamento do 2º é pelo NOME do passo via `normStepKey` (`lib/savedProducts.ts`) — a MESMA chave do produto salvo no passo.** Renomear `Step.title` na Rotina quebra o link **e** a foto do produto no passo, de uma vez.

**TRÊS abas (topo, estilo manhã/noite do protocolo):** **"Recomendados"** (a lista por passo acima — um passo pode vir preenchido pelo que ela **já tem em casa**, e aí o catálogo não entra nele), **"Minha Coleção"** (grade de quadrados com a barra de compatibilidade + editar/remover — ver "Feature: Minha Coleção") e **"Escaneados"** (histórico de `product_scans` da usuária, recarregado a cada vez que a aba abre — ⚠️ **esta aba continua sem cache de propósito**: as URLs das fotos são assinadas e expiram em 1h, então precisam ser regeradas; ver "CACHE DE DADOS → o que NÃO cachear"). Cada card escaneado = **foto real** que ela tirou (URL assinada do bucket privado `product-scans`, `cover`) + data + marca/nome + **chip de veredito** (verde/âmbar/vermelho) + resumo; toque → modal de detalhe com a análise completa (mesmo layout do `product-result`). Ver "Feature: Escanear Produto" abaixo.
...
**"Tenho esse produto em casa" → o produto entra na Minha Coleção:** ⚠️ **substituiu o "Salvar na minha rotina"**, que guardava a foto por passo em AsyncStorage (por aparelho). Hoje o botão do detalhe de um produto **recomendado** adiciona o produto à **Coleção no servidor** (`adicionarDoCatalogo`) e refresca a recomendação, então ele passa a ocupar o passo. Na tela de **Rotina (`protocolo.tsx`)**, cada passo com produto dela mostra a **foto no lugar do ícone** (quadradinho 46×46, `cover`), agora via **`lib/rotinaProdutos.ts`** (campo `em_casa` da recomendação), recarregando via `useFocusEffect`. A chave casa porque o `passo` da recomendação = o `name` do passo da rotina (= `Step.title`). Agora **sincroniza entre aparelhos**. Ver "Feature: Minha Coleção".
...
### Feature: Minha Coleção (o que ela tem em casa entra na rotina)

**O problema que resolve:** a IA montava a rotina "ideal" com produtos do catálogo sem saber o que a usuária já tem em casa — ela recebia uma **lista de compras**, não uma rotina para começar hoje. Agora a rotina usa **os produtos dela**, quando são compatíveis, e o catálogo só preenche **o que falta ou não serve**.

> ⚠️ **Estado: implementado, PENDENTE de deploy** (`recomendar-produtos`, `niks-chat`) **e da migration `20260922120000`**. Nada foi testado em device.

**A regra central:** a **estrutura** da rotina (quais passos existem, ordem, manhã/noite) continua sendo do `generate-protocol`, a partir do scan. A Coleção muda **só qual produto ocupa cada passo**. É isso que permite refazer a escolha de produto quando a Coleção muda **sem** reescrever a rotina — que precisa ficar estável ~30 dias para a usuária conseguir medir o que funcionou (ver o callout "Um novo scan NÃO regenera a rotina").

#### Persistência — tabela `colecao_produtos` (não é flag em `product_scans`)

Ver o schema em "Tabelas criadas". **Por que tabela nova:** `product_scans` é histórico **imutável** (a aba "Escaneados" mostra todo scan que já rodou); Coleção é **estado atual, mutável** (ela remove, refaz foto, a compatibilidade é recalculada). Com uma flag, "remover da Coleção" apagaria o histórico. Além disso o mesmo produto pode ter sido escaneado 2× (duas linhas) e ser **um** item da Coleção, e um produto **não identificado** nem chega a virar `product_scans` (a Edge Function retorna antes de persistir).

⚠️ **A escrita é do CLIENTE** (RLS por usuária: select/insert/update/delete das próprias linhas), ao contrário de `product_scans`. A análise continua sendo da Edge Function; o que o app grava é a declaração "tenho isso em casa" + a cópia denormalizada do veredito. Porta única de escrita: **`lib/colecao.ts`** — não escrever na tabela de outro lugar.

⚠️ **Remoção é sempre por `id` explícito** (`.in('id', ids)`), nunca por `user_id` — regra do projeto. Remover apaga **só** a linha da Coleção; o `product_scans` continua em "Escaneados".

#### Coleção × produto fixado — DOIS botões, duas tabelas, não misturar

| | **"Tenho esse produto em casa"** | **"Adicionar à minha rotina"** |
|---|---|---|
| Onde | resultado de um **scan** de produto (câmera **e** share) — pop-up em `product-result` | detalhe de um **recomendado do catálogo** na tela de Produtos — **e também o detalhe de uma alternativa** |
| Significa | "eu **tenho** esse produto" | "eu **quero** esse produto neste passo" |
| Grava em | `colecao_produtos` (Minha Coleção) | `rotina_produto_fixado` |
| Tem foto dela? | sim (bucket `product-scans`) | não — imagem pública do catálogo |
| Tem `compatibilidade`? | sim, medida pela `analisar-produto` | não se aplica |
| Vai para o chat? | **sim** (`<ProductsAtHome>`) | não |

⚠️ **Por que não é um botão só:** afirmar que ela TEM em casa um produto que ela só **escolheu** seria mentira — e a Coleção alimenta o NIKS Chat, que responde "o que eu faço agora?" com o que ela tem **em mãos**. Produto do catálogo **não entra na Coleção** (todo item da Coleção passou por uma foto que ela tirou).

**As duas origens de um item da Coleção:** scan de produto (pop-up) e o lote do primeiro fluxo. Nos dois a foto é dela e a compatibilidade é medida.

#### Primeiro fluxo (uma vez por CONTA)

**Gatilho:** o que vier primeiro entre a **primeira abertura da aba Rotina** e o **primeiro toque no "Escanear" da home**. ⚠️ No caso do "Escanear", `useFaceScan().startFaceScan()` **não abre a câmera**: leva para `/protocolo`, e o fluxo começa lá — assim existe **um só lugar** que faz a pergunta (`hooks/useColecaoOnboarding.tsx`, montado no `protocolo.tsx`), não dois.

```
pop-up "Você tem produtos de skincare em casa?"
  ├─ Sim → (scan)/colecao-camera  → fotografa TODOS em sequência (fila no store)
  └─ Não → (nada é enfileirado)
         ↓ os dois caminhos seguem para o mesmo lugar
  scan de pele DE DENTRO do app (scan-prep-app/camera-multi, 6 fotos)
         ↓ loading-dentro-app salva o skin_scans
  lib/colecaoFlow.ts#finalizarPrimeiroFluxo (BACKGROUND):
    1. analisa a fila → Coleção        2. regenera a rotina (que já vê a Coleção)
    3. se a rotina não foi regenerada mas a Coleção mudou → refresca só a recomendação
```

> ⚠️ **POR QUE A FILA ESPERA O SCAN (ponto que não é óbvio).** A `analisar-produto` calcula a compatibilidade contra o **último `skin_scans`**. No primeiro fluxo os produtos são fotografados **antes** do rosto — analisar na hora mediria os produtos contra o scan do **onboarding** (1 foto) e exigiria recalcular tudo depois: **o dobro de chamadas de visão por produto**. Guardando as fotos e analisando **depois** do scan, cada produto custa **uma** chamada e já nasce medido contra o retrato bom da pele. A espera não é da usuária — ela já está vendo o resultado do scan enquanto isso roda.

> ⚠️ **O scan é o de DENTRO do app, nunca o do onboarding:** o do onboarding usa 1 foto e existe para conversão; o do app usa 6 e é o retrato confiável. A compatibilidade dos produtos de casa precisa ser calculada contra esse.

> ⚠️ **Este fluxo SUBSTITUIU o gatilho da regeneração única do 1º scan in-app.** `loading-dentro-app` chama `finalizarPrimeiroFluxo` **no lugar de** `regenerateProtocolInApp` — que continua sendo o único caminho de regeneração (com os mesmos guards `inapp_protocol_regenerated_at` + `regenInFlight`), só que agora envelopado. **Não criar um segundo caminho de regeneração.** Sem fila e sem fluxo pendente, o comportamento é idêntico ao de antes.

> ⚠️ **O marcador `users.colecao_onboarding_at` é gravado quando ela RESPONDE o pop-up, não no fim do fluxo.** Parece errado, mas: **todo** scan de 6 fotos passa pelo `finalizarPrimeiroFluxo`, inclusive o de quem nunca viu o pop-up porque o gate falhou por rede — marcar lá esconderia o fluxo dessa pessoa para sempre. Custo aceito: app morto entre a resposta e o fim do fluxo **perde a fila de fotos**, e ela adiciona os produtos pela aba Minha Coleção (o mesmo caminho de recuperação que a tela já oferece).

> **Quem vê:** mesma arquitetura do tutorial da home (Sessão 64) — a **verdade** é `users.colecao_onboarding_at`; `colecaoOnboardingDone` no store é **cache do aparelho** (persistido, e **zerado no logout** por `clearColecaoFlags`, senão o "já fez" de uma conta vazaria para a próxima criada no mesmo aparelho).

> **Teto: 12 produtos por sessão** (`MAX_PRODUTOS_PRIMEIRO_FLUXO`). Cada produto = 1 chamada de visão, e o risco nº 1 da feature é abandono num fluxo longo (produtos + 6 fotos do rosto). Ao bater o teto a câmera avisa que dá para completar depois pela aba.

#### Estados especiais de item

- **Incompatível** = `veredito === 'evitaria'` (`VEREDITO_INCOMPATIVEL` em `lib/colecao.ts`, com **gêmeo na `recomendar-produtos`** — mudou num, mude no outro). Continua **visível** na Coleção, marcado "Não recomendado pra sua pele" **+ o motivo vindo da análise**, e fica **fora da rotina**. ⚠️ `com_ressalva` **continua elegível** (ranqueia abaixo pela compatibilidade, que é numérica): a ideia é usar o que ela tem, não exigir perfeição.
- **Não identificado** (`status: 'nao_identificado'`) = a IA respondeu `precisa_foto`. Fica **fora da rotina** — não dá para avaliar o que a IA não leu — com o caminho de recuperação **"Tirar nova foto"** (inclusive do rótulo de ingredientes). ⚠️ Como a Edge Function **não persiste nada** nesse caso, **o app sobe a foto** (policy nova de INSERT no bucket `product-scans`, migration `20260922120000`) — sem isso o card nasceria sem imagem e ela não saberia o que refotografar. A refoto **atualiza o item existente** (`colecaoRetakeId` no store → `identificarItem` no `product-result`), nunca cria outro: senão cada tentativa deixaria um card morto.
- **Erro de rede na análise da fila** cai no **mesmo tratamento** do não identificado. A foto não pode simplesmente sumir.

#### Como a Coleção entra na rotina (`recomendar-produtos`)

**Ordem de resolução de um passo — a escolha dela ganha da IA:**

```
1. produto FIXADO por ela  (rotina_produto_fixado)   → `fixado: true`
2. produto da COLEÇÃO      (colecao_produtos)        → `em_casa: {...}`
3. escolha anterior        (só com preservar_catalogo)
4. catálogo via IA         (ou `sem_produto`, ou omitido)
```

⚠️ **Passo 1 antes do 2, de propósito:** se ela fixou um recomendado e depois um produto de casa compatível aparece para o mesmo passo, **a escolha dela permanece** — a IA não troca por baixo. E o item da Coleção não é consumido: continua disponível para outro passo.

⚠️ **O fixado é REVALIDADO contra o pool elegível** (categoria, gestante, alérgeno e, em Tratamento, o ativo-alvo) antes de ser aplicado. Ele foi escolhido dentro das regras de então, mas o perfil dela pode ter mudado (gravidez, alergia nova) ou o produto pode ter saído do catálogo. Não passando, o passo cai no fluxo normal e o caso é logado — **segurança clínica não é negociável nem pela escolha dela**.

Um passo resolvido por (1) ou (2) **nem chega à camada de IA** — não faz sentido escolher produto do mercado para um passo já resolvido, e economiza tokens. No caso (1) as **alternativas** do card vêm das que já estavam no passo (preservando a copy) ou do topo do pool elegível, para o card não virar um item solto. Gates aplicados ao produto da Coleção, na ordem:

1. **categoria** do item == categoria do passo (`categoriaDoItem` reusa o **mesmo `classifyStep`** dos passos — mesma régua dos dois lados);
2. **gestante/lactante** → nada de retinoide;
3. **corte de alérgeno** pelo que dá para ler nos `ativos_detectados` (cinto-e-suspensório: a `analisar-produto` já conhecia as alergias dela pelo context pack; fragrância/álcool não aparecem como ativo e não são alcançáveis aqui);
4. ⚠️ **trava clínica de manhã** — retinoide/AHA **nunca** ocupam passo AM. É sobre o **produto**, não sobre o passo: o passo pode ser um hidratante da manhã e o creme dela conter retinol;
5. **não dobrar família forte** (retinoide/AHA/BHA) no mesmo período;
6. passo de **Tratamento** exige o ativo que o passo pede (mesmo gate do catálogo: o passo de vitamina C não é preenchido pelo azelaico que ela tem).

**Dois ou mais compatíveis para o mesmo passo** → entra o de **maior compatibilidade** (a lista vem ordenada desc; itens do catálogo, sem número, ficam por último). A decisão é sempre da IA/motor — a usuária não escolhe. Um item só ocupa **um** passo.

> ⚠️ **`preservar_catalogo: true` — não remover.** Quando a Coleção muda, `recomendar-produtos` é rechamada com `regenerate: true`. Sem essa flag a camada de IA seria re-rodada para **todos** os passos e — como o modelo não é determinístico — **adicionar um produto trocaria em silêncio os recomendados de passos sem relação nenhuma**. Com ela, passos intocados mantêm exatamente o que tinham (e, se nada sobrar para escolher, a chamada ao modelo nem acontece). Use `false` só quando a **rotina** mudou (regeneração do protocolo), aí os passos são outros. Quem decide é o parâmetro `preservarCatalogo` de **`lib/recomendarProdutos.ts`** (a chamada mora lá, fonte única).

**Formato no JSON de `recomendacoes_produtos`:** o passo ganha `em_casa: { colecao_id, nome, marca, image_path, produto_id, compatibilidade, veredito, copy }` e `produtos: []`. A foto sai de `image_path` (bucket privado → URL assinada na leitura, **nunca cacheada**) ou, no caminho do catálogo, de `produto_id` → `produtos.imagem_url`.

#### Compatibilidade também nos produtos do CATÁLOGO

Todo produto recomendado passa a ter nota 0–100, calculada **na geração** e guardada em `recomendacoes_produtos` (`produtos[].compatibilidade` + `veredito`) — abrir a tela não custa IA.

⚠️ **A régua é a MESMA da Coleção, e isso não é detalhe de implementação — é o requisito.** Os dois números aparecem lado a lado na rotina e a usuária decide comparando; duas réguas com a mesma cara mentiriam para ela. Por isso a Camada 1 do prompt clínico (cortes duros + veredito + faixas do número) foi **extraída para `_shared/compatibilidade.ts`** e a `analisar-produto` passou a **interpolar** esse texto em vez de ter a própria cópia. Conferido na extração: o prompt em runtime da `analisar-produto` ficou **byte a byte idêntico** ao de antes.

**Por que extrair em vez de dar um 2º modo de entrada à `analisar-produto`:** aquela função é moldada em torno de **uma foto** — exige imagem, persiste em `product_scans`, tem o caminho `precisa_foto`, devolve Camada 2 e cinco campos de prosa. Nada disso serve para produto de catálogo, que precisa **só** do número + veredito; um flag de modo forkaria a função em ~6 pontos.

**Custo e latência (medidos antes de implementar):**
- **1 chamada em lote** para todos os produtos: ~2,7k tokens de entrada + ~430 de saída. Produto a produto seriam **~61k** (o system prompt de 3,4k e o context pack repetidos N vezes) — por isso é lote.
- É ~50% em cima da chamada de seleção que a função já faz, **uma vez por usuária**. Menos de um centavo; ordem de grandeza abaixo de um scan de pele.
- **Chamada separada da que escolhe os produtos, de propósito:** o prompt clínico proíbe o número de justificar a decisão de rotina; juntar seleção e pontuação no mesmo pedido convidaria o modelo a alinhar as duas coisas.
- **Mesmo modelo da `analisar-produto` (`gpt-5.4-mini`)**, embora `gpt-4.1-mini` fosse mais barato: mesma rubrica em modelo diferente calibra diferente, e é a comparabilidade que se está protegendo. ⚠️ Teto **8192** — é modelo de raciocínio, e 4096 já truncou a `analyze-skin`.
- **Latência: zero para a usuária.** A pontuação roda em `waitUntil` DEPOIS da resposta. A tela mostra a recomendação assim que ela existe e, enquanto faltar nota, refaz a busca a cada 5 s (máx. ~30 s, leitura de banco, sem IA). Sem nota → a barra simplesmente não é desenhada.

#### Produto reprovado nunca é recomendado (corte clínico no catálogo)

**Bug medido em produção:** "Some By Mi AHA-BHA-PHA Clear Foam" recomendado com **compatibilidade 25%** (faixa `evitaria`). A causa não era só a ordem do pipeline: a régua clínica tem **quatro** cortes duros (alérgeno, gravidez, **barreira comprometida**, **rosácea**) e o filtro SQL do `poolFor` conhecia **dois**. Barreira e rosácea eram lidas por `scanConcernCodes` só para **ranquear**, nunca para **excluir** — o dado estava na mão e não cortava nada.

**Conserto em duas metades (híbrido):**

1. **Corte determinístico ANTES da seleção** (`reprovadoPorCorteClinico` no `poolFor`): cruza `ativos_principais`/`alergenos` do produto com `barrier_status` e `rosacea.present` do scan. Custo zero de IA, latência zero, e mata a causa estrutural. ⚠️ `levemente_comprometida` **não** corta — a régua fala em "comprometida/severamente comprometida".
2. **Descartar-e-promover DEPOIS da pontuação** (`aplicarCorteClinicoNoPasso`): quem ficou em `evitaria` sai, e o de maior nota entre os sobreviventes vira principal. ⚠️ **Sem chamada de IA nova** — principal e alternativas já foram pontuados no mesmo lote; trocar pelo "próximo do pool" exigiria repontuar, iterativo e de custo imprevisível.

**O que o corte determinístico NÃO alcança, de propósito** (fica para a rede da pontuação): "niacinamida >5%" (o catálogo não guarda concentração), "vitamina C L-AA puro" (o código é só `vitamina_c`, sem distinguir de SAP/MAP, que a régua **permite** — cortar tudo tiraria produto válido) e "esfoliante físico" (a categoria `esfoliante` sequer é recomendada hoje). **Sub-cortar aqui é a escolha certa**: o que escapa é pego pela nota.

**O corte é pelo VEREDITO** (`VEREDITO_INCOMPATIVEL` — a mesma constante que tira produto da Coleção da rotina), com **`< 40`** como rede para linha sem veredito (40 é o piso de `com_ressalva`). Produto **sem nota nenhuma** não é reprovado: pontuação que ainda não chegou não pode sumir com produto da tela.

⚠️ **Vale inclusive para passo `fixado`** — segurança clínica não é negociável nem pela escolha dela. Mas: **fixado que SOBREVIVE não é rebaixado** por uma alternativa de nota maior (isso seria a IA trocando por baixo, o que `fixado` existe para impedir); fixado reprovado é promovido para fora e o passo perde o `fixado` **nesta** recomendação — a linha em `rotina_produto_fixado` permanece, e o produto volta a ser dela se voltar a ser elegível numa geração futura.

⚠️ **NÃO se aplica à Minha Coleção.** Produto dela já foi avaliado por FOTO com estes mesmos cortes; o veredito guardado é que decide, e produto incompatível dela **fica na Coleção marcado, fora da rotina** — comportamento próprio, inalterado.

> ⚠️ **INVARIANTE QUEBRADA DE PROPÓSITO — estado vazio clínico vale para STAPLE.** A regra era "staple (limpeza/hidratante/protetor) nunca vira `sem_produto`, some em silêncio", porque a causa era **lacuna de catálogo** — problema nosso, que não informa nada a ela. Quando quem esvazia o pool é o **corte clínico**, a causa é sobre a **pele dela** e sumir com o passo esconderia o recado que mais importa ("não use ácido agora"). Por isso o JSON ganhou `motivo_clinico: true` + `titulo`, e o `poolFor` devolve `cortadosPorClinica` — é esse número que distingue os dois casos.

**Copy do estado vazio clínico** (`copiaEstadoVazioClinico`, fonte única na Edge Function — a tela só renderiza). Tom: informar sem alarmar; "nenhuma limpeza serve pra você" soa como veredito sobre a pele dela, o recado certo é sobre o **momento**. ⚠️ **Nenhuma variante promete que o produto volta sozinho quando a pele melhorar** — a rotina é congelada e isso não acontece (ver "CONSEQUÊNCIA CONHECIDA"); por isso o convite é o **chat**, que é o caminho real de reavaliação.

| Situação | Título | Corpo |
|---|---|---|
| Barreira | "Sua barreira está em recuperação" | "Enquanto ela se refaz, ácido e esfoliante atrasam o processo em vez de ajudar — melhor não usar agora. Deixei este passo de fora por enquanto; quando ela estiver firme, me chama no chat que eu ajusto." |
| Rosácea | "Sua pele está mais reativa agora" | "Com sinais de rosácea, ácido e fragrância costumam acender a vermelhidão — melhor evitar neste passo por enquanto. Me chama no chat se quiser uma alternativa suave." |
| Os dois | "Sua pele pede um intervalo aqui" | "Sua barreira está em recuperação e sua pele mostra sinais de rosácea. Ácido, esfoliante e fragrância agora atrapalhariam os dois, então deixei este passo de fora por enquanto. Me chama no chat que a gente vê uma alternativa suave." |
| Residual (corte veio da nota) | "Nada que eu recomende aqui agora" | "Não encontrei um produto que combine com a sua pele neste momento. Prefiro não indicar nada a indicar algo que pode irritar." |

> ⚠️ **A nota é de QUANDO a recomendação foi gerada, não de agora.** Ela é calculada uma vez e guardada; a rotina é congelada em seguida. Logo, um produto cortado (ou aprovado) sob uma condição transitória — barreira comprometida, rosácea ativa, gravidez — **não é reavaliado sozinho** num scan posterior. Ver a "CONSEQUÊNCIA CONHECIDA" no callout "Um novo scan NÃO regenera a rotina", na tela de Recomendação de Produtos: é comportamento intencional, não bug.

> ⚠️ **Diferença de evidência, conhecida e DELIBERADAMENTE invisível na interface.** A nota da Coleção nasce de uma **foto** (a IA lê o rótulo real, com concentração); a do catálogo nasce da **ficha** (`ativos_principais`, sem concentração). Mesma régua, qualidades de evidência diferentes — o produto de catálogo tende a pontuar de forma mais "limpa" por não ter o ruído de uma foto mal lida. **Decisão de produto: NÃO sinalizar a origem na tela.** Saber que uma nota veio de foto e outra de ficha não ajuda a usuária a decidir — só planta dúvida sobre o número. A diferença mora no código (`_shared/compatibilidade.ts`) e aqui, não na interface.

#### Onde aparece no app

- **Barra de compatibilidade no DETALHE do produto**, logo abaixo do nome — mesma barra e mesmas cores da Minha Coleção (`getScoreTheme`). Vale para produto do catálogo **e** para produto dela (`em_casa`), pelo mesmo campo: é a comparação lado a lado que a feature existe para permitir.
- **Aba "Minha Coleção"** (3ª aba de `recomendacao-produtos.tsx`, ao lado de Recomendados e Escaneados): **grade de quadrados** com a foto + **barra horizontal 0–100%** da compatibilidade, colorida por **`getScoreTheme`** — a **mesma régua** do Niks score e do resultado de produto (0–25 vermelho · 26–50 laranja · 51–75 amarelo · 76–100 rosa). ⚠️ **Não criar régua nova.** Sem cache, pelo mesmo motivo da aba "Escaneados" (URLs assinadas expiram em 1h).
- **Editar → selecionar → Remover** (com confirmação; é destrutivo e não tem desfazer). ⚠️ **Não existe "marcar como acabou"** nem edição de dados do produto — decisão consciente. Consequência aceita: produto que acabou e não foi removido continua na rotina.
- **Detalhe de um item** reusa **`components/product/ProductAnalysis`** (fonte única do layout). Item não identificado não tem análise: vira um cartão curto com o caminho de recuperação.
- **Rotina (`protocolo.tsx`):** a foto **e o subtítulo** do passo vêm de **`lib/rotinaProdutos.ts`** (`getProdutosDosPassos`), que resolve só os passos com produto **ESCOLHIDO por ela** — `em_casa` (foto dela, assinada) ou `fixado` (imagem do catálogo). ⚠️ **Mera recomendação da IA não entra**: a Rotina mostraria um produto que ela não escolheu, e a foto daria a entender que ela tem em casa. ⚠️ **O subtítulo do card passou a ser `marca + nome do produto` quando há produto escolhido**, caindo no `ingredient` do protocolo quando não há — ver "Subtítulo do passo" na seção da tela de Rotina. ⚠️ **`lib/savedProducts.ts` foi APOSENTADO** — sobrou só o `normStepKey` (chave de casamento passo↔produto, usada em 3 lugares). Os dados antigos em AsyncStorage (`saved_routine_products_v1`) **não são migrados**: guardavam só a foto de um produto do catálogo por passo, sem id nem compatibilidade — não dá para deduzir deles que ela "tem o produto em casa".
- **Botão do detalhe de um recomendado (e da ALTERNATIVA dele):** era "Salvar na minha rotina" (AsyncStorage, por aparelho), virou **"Adicionar à minha rotina"** → fixa o produto no passo (`rotina_produto_fixado`), **fecha o detalhe e leva para a Rotina** com o passo já mostrando o produto escolhido. ⚠️ **A navegação ESPERA a `recomendar-produtos` responder** (o botão fica em "Na sua rotina" com spinner): é a recomendação que diz à Rotina qual produto ocupa o passo, e navegar antes abriria a Rotina com o produto antigo — o `useFocusEffect` de lá já teria rodado e nada corrigiria a tela. O `router.push` sai **250 ms depois** de fechar o modal (mesmo respiro do ScanModal: empilhar navegação sobre `<Modal>` nativo em dismiss trava a tela no iOS). ⚠️ O detalhe da alternativa **não tinha botão nenhum** antes; agora tem o mesmo. Estado "Na sua rotina" vem do próprio JSON (`fixado`), sem ida extra à rede.
- **Pop-up depois de QUALQUER scan de produto** (câmera e share): mora em **`app/(scan)/product-result.tsx`**, **1 s** depois de a tela montar. ⚠️ **Não colocar dentro do `ProductAnalysis`** — o componente é compartilhado com o detalhe do histórico, e o pop-up apareceria ao abrir um scan antigo. Os dois caminhos de scan terminam no `product-result`, então uma vez só cobre os dois. "Não" → o produto fica só em "Escaneados", como sempre foi.

#### NIKS Chat

A Coleção entra no context pack como **`<ProductsAtHome>`** (ver `niks-chat/context.ts` + `prompt.ts`), para a NIKS responder "apareceu uma espinha, o que eu faço?" **com o que ela tem em mãos** em vez de mandar comprar. ⚠️ O bloco é **sempre** emitido, inclusive afirmando que a coleção está vazia — mesma lição do `<PendingProtocolSuggestion>`: sem o estado explícito, a NIKS lia a própria fala anterior no histórico e inventava. Itens **incompatíveis vão junto** (ela precisa saber que existe e explicar por que não manda usar); **não identificados ficam de fora** (não há o que dizer sobre o que não foi lido).

#### Fora do escopo (decidido)

Wish list; "marcar como acabou"; editar dados de um produto; regenerar a rotina inteira a cada produto adicionado.

#### Instrumentação

`colecao_primeiro_fluxo_perguntado` · `colecao_primeiro_fluxo_respondido {tem_produtos}` · `colecao_primeiro_fluxo_abandonado {total}` · `colecao_fotos_enfileiradas {total}` · `colecao_lote_analisado {enfileirados, identificados, nao_identificados}` · `colecao_pergunta_respondida {tem_em_casa, compatibilidade, veredito}` · `colecao_item_adicionado {origem: scan}` · `colecao_item_identificado` · `colecao_itens_removidos {total}` · `rotina_produto_fixado {origem}` · `colecao_rotina_carregada {passos_total, passos_com_produto_dela}` — este último é **a métrica central da feature**: quantos passos acabaram preenchidos com produto que ela já tinha.

### Feature: Compartilhar com o NIKS (Share Extension iOS → análise de produto)

A usuária, navegando no Mercado Livre, Safari ou site de marca, toca em **Compartilhar → NIKS** e recebe a análise de compatibilidade do produto — sem precisar ter o produto em mãos. Entradas: **link** (caso principal — o Mercado Livre compartilha link, não imagem), **imagem** (segurar a foto no Safari → compartilhar) e **texto com URL**. O resultado é a MESMA tela `product-result` (via `ProductAnalysis`) e fica salvo em `product_scans` (aba "Escaneados"), como um scan normal. ⚠️ **Estado: implementado, PENDENTE de validação no device e de deploy da `extrair-imagem-produto`.**
...
2. `app/_layout.tsx` → `ShareIntentProvider` (por fora de tudo, `scheme: 'niks-ai'`) + `ShareIntentBridge` dentro do `AppShell`. O despachante de deep link **ignora `dataUrl=`** (não manda pro Superwall).
3. `components/share/ShareIntentBridge.tsx` → normaliza (`lib/shareProduct.ts#normalizeShareIntent`: arquivo de imagem > **link que já é imagem** (`kind: 'image_url'`, ver abaixo) > `webUrl` > URL no texto), grava **`pendingShare`** no store e chama `resetShareIntent()`. **Nunca navega.** Copiar na hora é obrigatório: o hook do pacote zera o conteúdo quando o app vai para background (`resetOnBackground`).
4. `app/(app)/_layout.tsx` → efeito `[ready, needsName, pendingShare]`: **só com `ready`** (sessão + assinatura verificadas) faz `router.push('/(scan)/share-product-loading')`. Guard `useRef` por `pendingShare.id` (decisão 25). Antes do push, garante que **nada fica por cima**: (a) se um seletor nativo estiver aberto (`lib/nativePresentation.ts` — home e niks-chat registram suas chamadas de galeria/câmera), **espera fechar** (o JS não consegue fechar um `PHPickerViewController`); (b) se o topo não for do grupo `(app)`, `router.dismissAll()`. Os dois casos logam `console.warn` + Mixpanel `product_share_blocked_by_modal { tipo }`.
> ⚠️ **Por que o consumo mora no `(app)/_layout` e não na tela:** o grupo `(scan)` **não tem guard nenhum** (Stack puro) — uma tela de `(scan)` aberta direto renderiza sem checar assinatura. Sem sessão ou sem assinatura, o guard manda pra welcome/paywall como sempre; o `pendingShare` espera em memória e é consumido quando ela chega em `(app)` depois de login/paywall (todos terminam em `/(app)/home`). **O share nunca pula o paywall.** Limitação aceita: app morto no meio do login/paywall perde o share (não persistido — hand-off de vida curta).

**Tela `app/(scan)/share-product-loading.tsx`** (exclusiva da feature, fora do `(app)` → sem navbar, `gestureEnabled: false`). Lê `pendingShare` UMA vez na montagem e limpa o store; sem share → `replace('/(app)/home')`.
- **Visual:** frase no topo (Nunito ExtraBold 22, fade) → círculo: halo rosa pulsando + trilho + **arco de ~72% em gradiente `#FFC9C9→#FF9D9D` girando contínuo** (reanimated, 1,4 s/volta) + disco interno 200pt com **crossfade rosto ↔ produto** (~2 s cada, fade 600 ms) → `%` discreta embaixo. Aviso de alta demanda igual ao da `product-loading`.
- **Rosto:** `lib/facePhoto.ts#getFacePhotoUrl` = **mesma regra da home** (`users.foto_home_url ?? último skin_scans.foto_url`), aproveitando o cache `home:${uid}` se estiver em memória. Sem foto (ou URL expirada → `onError`) → só o produto, sem alternar.
- **Produto:** imagem → pronta na hora. Link → **placeholder** (logo sparkle tintada `#FF9D9D`, respirando) até a foto chegar; aí entra com fade e começa a alternar.
- **Foto do link — três fontes, nesta ordem:** (1) **`pendingShare.pageImageUrl`**, lida **no próprio Safari** pelo preprocessor da extensão → o app baixa (`lib/shareProduct.ts#prepareProductImageFromUrl`, só https) e **pula a Edge Function**; (2) `extrair-imagem-produto`; (3) WebView invisível.
> 🔍 **PENDÊNCIA — a fonte (1) quase nunca dispara (set/2026).** Medido com log temporário na ponte: em share de página do Safari o `shareIntent.meta` chegou ao app **com ZERO chaves** (`niks:image` e `og:image` ausentes), e o log da extensão **não tinha** `NSExtensionJavaScriptPreprocessingResultsKey` naquele share — ou seja, o iOS não rodou o preprocessor. Em OUTROS shares (Mercado Livre pelo Safari) a mesma linha apareceu com `baseURI` + `meta` cheios, então o preprocessor funciona às vezes. **Hipótese a investigar:** num share de página o iOS manda DOIS attachments (o `propertyList` com o resultado do JS e o `public.url` cru); o `ShareViewController` do pacote grava os dois no mesmo array e o `parseShareIntent` lê só `weburls[0]` — se o `public.url` (com `meta: ""`) vier primeiro, o `niks:image` é perdido. Consequência prática hoje: **o fluxo cai no backend**, que funciona para as lojas testadas, então a feature não está quebrada — só mais lenta do que poderia, e sem a vantagem de ler pelo aparelho nas lojas que bloqueiam o servidor. Para medir de novo: logar `Object.keys(shareIntent.meta).length` no `ShareIntentBridge`. Mixpanel `product_share_image_extracted { fonte: 'safari' | 'backend' }`. ⚠️ Só existe quando o share vem de uma **página** (Safari/SFSafariViewController). O app do Mercado Livre compartilha **texto com link** → não há metatags → sempre cai no backend.
- **Frases:** link começa em "Buscando o produto…" (a busca ocupa 0–14%; a análise continua de 15%) → "Analisando os componentes do produto…" → "Comparando com a sua pele…" → "Vendo se o produto é compatível com você…" → "Montando o veredito…". Imagem começa na 2ª.
- **✅ REVELAÇÃO (a tela NÃO navega sozinha):** terminada a análise, o anel fecha em 100% e, meio segundo depois, a tela **vira a revelação**: número da compatibilidade grande acima do círculo (`Nunito ExtraBold 72`, `letterSpacing -2.88`, cor por faixa via `getScoreTheme`) + "Compatibilidade de pele" (`ExtraBold 20`, `marginTop -14`) — os MESMOS valores do `product-result` — e o botão **"Ver análise completa"** embaixo (pílula `#FF9D9D`, `height 60`). Somem a frase rotativa, o subtítulo e a % pequena; o círculo continua alternando. Entrada com fade + slide-up escalonado (número 420 ms; botão com 160 ms de atraso). **Nada além do número** — chip de veredito e nome do produto são do `product-result`. Sem `compatibilidade` → "—" e tema rosa, com o botão funcionando igual. ⚠️ Feito com `useProductAnalysis({ navigateOnSuccess: false })`: **a tela da câmera (`product-loading`) não muda**, continua navegando sozinha.
- **Ritmo da alternância:** parada de **1300 ms** em cada foto + crossfade de **350 ms** (2000 ms parecia slideshow, 900 ms ficou afobado — 1300 ms foi o ponto escolhido no device).
- **Falhas:** link sem foto → "Não conseguimos pegar a foto desse link. Tente compartilhar a imagem ou tirar uma foto." + **Tirar foto** (`product-camera`) / Voltar; erro de rede na busca → retry da busca; erro da análise → `retry()` do hook.
- **Consentimento de IA:** `useScanConsentGate({ onDecline, presentation: 'inline' })` — a imagem só é preparada (e a IA só é chamada) **depois de `granted`**. Recusar → `back()` ou home (não há câmera por trás). ⚠️ **`presentation: 'inline'` não é preferência visual:** com um `<Modal>` nativo, se houver outro controlador apresentado (ex.: a galeria da foto da home aberta quando o share chega), o iOS recusa com `Attempt to present … which is already presenting <PHPickerViewController>`, o modal **nunca** aparece e a tela fica presa em 0% para sempre (bug real, set/2026). No modo inline o aviso é uma camada absoluta DENTRO da tela. **As câmeras continuam no `<Modal>` nativo.**
- **⚠️ Link que JÁ É uma imagem (`kind: 'image_url'`)** — `…/foto.png?v=1`. Acontece quando a usuária usa "Compartilhar imagem" no toque longo, ou quando o Safari está exibindo o arquivo (a galeria do Shopify abre a foto numa URL própria). Nesse caso **não existe página**: o iOS não roda o preprocessor (sem `NSExtensionJavaScriptPreprocessingResultsKey`), `meta` vem vazio e o app baixa a foto direto, sem backend e sem WebView. Bug real (set/2026): a Sallve compartilhada assim ia para a WebView, que passava 25 s reinjetando script num PNG enquanto a tela ficava em 11% com o placeholder. `isDirectImageUrl` também **impede a WebView** em qualquer URL de imagem.
- **Imagem:** as fotos do share (imagem compartilhada, Safari, WebView e backend) passam TODAS por `lib/shareProduct.ts#prepareProductImage`, que **normaliza para QUADRADO 512×512 com fundo branco**, produto centralizado e ~8% de margem. ⚠️ **Motivo:** foto de loja costuma ser bem vertical (a do Mercado Livre veio 316×1200) e o círculo do `product-result` usa `cover` — sem normalizar, o frasco sai cortado no meio. Como a `analisar-produto` salva a `images[0]` no bucket, o histórico ("Escaneados") fica com a mesma versão quadrada. ⚠️ **Feito com Skia fora da tela** (`Skia.Surface.MakeOffscreen` + `drawImageRect`), **não** com o `extent` do `expo-image-manipulator` — esse action é `@platform web` e não faz nada no iOS. Falha do Skia → cai no downscale simples (512px, JPEG 0.5). A **câmera não passa por aqui** (a foto já é enquadrada pela usuária). Viaja pelo store (`setProductImage`); link e título em **`productSourceUrl`/`productSourceTitle`** (campos separados). O título vai como `productName` para a IA.

> 🔍 **PENDÊNCIA — scan salvo sem a usuária ver o resultado (set/2026).** Em **2 das tentativas** de teste, a `analisar-produto` respondeu `ok` e **gravou a linha em `product_scans`**, mas a tela de carregamento foi desmontada antes de navegar para o `product-result`: a usuária voltou para a home sem ver nada (e gastou uma chamada de IA). Nos dois casos havia **Fast Refresh** editando arquivos no meio do fluxo, e a mitigação já entrou (`lib/shareResume.ts` — a tela retoma em vez de ir para a home). **O que falta:** confirmar se o mesmo desmonte pode acontecer **fora do desenvolvimento** (remount do Stack, memory warning, iOS matando a cena durante a análise) e, se puder:
> - garantir que a tela **retome** (hoje a retomada cobre o remount, mas não um desmonte definitivo no meio da chamada);
> - ou, no mínimo, que o scan já salvo seja **oferecido à usuária** — a linha aparece na aba "Escaneados" na próxima vez que ela abrir a tela, mas **ninguém avisa que a análise ficou pronta**.
>
> Investigar com o `clientScanId` (já é estável e idempotente): dá para, ao montar sem resultado, perguntar ao banco se aquele `client_scan_id` já tem análise salva e abrir o resultado direto, sem nova chamada de IA.
...
>
> **Como testar no simulador:** `npx expo run:ios` logada numa conta assinante → `xcrun simctl pbcopy booted <<< '<link do ML>'` → app **Notas** do simulador → colar, selecionar o texto → Compartilhar → NIKS (mesmo caminho do app do ML: texto com link, sem metatags) → acompanhar `[share-webview]` no Metro.

**OAuth do Mercado Livre — decisão (set/2026): NÃO implementar agora** — fica como opção futura. Até lá, link do app do ML cai em `pagina_bloqueada` → "tente compartilhar a imagem ou tirar uma foto".

> **Opção futura — OAuth do Mercado Livre (passo a passo para cadastrar o app).** Fonte: [Crie uma aplicação](https://developers.mercadolivre.com.br/pt_br/crie-uma-aplicacao-no-mercado-livre) e [Autenticação e autorização](https://developers.mercadolivre.com.br/pt_br/autenticacao-e-autorizacao).
> 1. Entrar no **DevCenter** do ML com a **conta do NIKS** (não de usuária). Os dados do titular precisam estar validados — no Brasil só é permitido **1 aplicação por conta**.
> 2. **Criar uma aplicação** e preencher: **Nome** (único), **Nome curto**, **Descrição** (até 150 caracteres), **Logo**.
> 3. **URI de redirecionamento**: HTTPS e fixa (sem parâmetros variáveis).
> 4. **PKCE**: ativar (recomendado).
> 5. **Escopos**: só **Leitura**. **Tópicos/notificações**: nenhum.
> 6. **Unidade de negócio**, se o formulário pedir: **Mercado Livre** — desde 30/08/2026 apps de Mercado Livre e Mercado Pago precisam ser separados.
> 7. Guardar **App ID** e **Secret Key** como secrets do Supabase (nunca no app, nunca em chat).
> 8. Implementação (quando decidirmos): autorizar **uma vez** com a conta do NIKS (`authorization_code`) e manter o **`refresh_token` rotativo** numa tabela do servidor (só o último vale; access token dura 6 h). **Não existe `client_credentials`** no ML.
> 9. ⚠️ Validar antes de investir: a doc lista **"IP bloqueado"** entre as causas de 403 — nem com token é garantido que os IPs do Supabase passem. Testar `/items/{id}` e `/products/{id}` com o token de dentro de uma Edge Function antes de ligar no app.

**⚠️ Gotcha do Podfile (pré-existente, não é da feature):** o `ios/Podfile` **não aplica** os `extraPods` declarados no `app.json` via `expo-build-properties` — o `Podfile.properties.json` traz `apple.extraPods`, mas o Podfile gerado não lê essa chave. Resultado: `pod install` falhava com *"The Swift pod `AppCheckCore` depends upon `GoogleUtilities` and `RecaptchaInterop`, which do not define modules"*. **Correção aplicada à mão** dentro do `target 'NIKS'` do `ios/Podfile`:
> ```ruby
...
> ```
> Conferir com `otool -l <binário> | grep -A4 __entitlements` (no simulador os entitlements ficam na seção `__TEXT,__entitlements` do binário, **não** na assinatura — `codesign -d --entitlements` mostra vazio e engana).

**Lojas testadas (set/2026, curl + simulador):** ✅ **Creamy** 1080×1080 · ✅ **Sallve** 1000×1000 (só depois da correção da variante `_small`) · ✅ **Época** 1000×1000 · ✅ **Amazon** 1000×1000 · ✅ **Mercado Livre** só pela WebView (~1,8 s) · ❌ **Shopee**: a página é montada por JS (backend → `sem_metatag`) e **dentro da WebView ela redireciona para `/verify/traffic/error`** — não há caminho por link, a usuária precisa compartilhar a imagem ou tirar foto.

**Riscos conhecidos:** Mercado Livre pode bloquear scraping de IP de datacenter (mitigado pela API pública, que também pode passar a exigir token); `og:image` pode ser banner/kit (JSON-LD `Product` tem prioridade); redirect por JavaScript não é seguido (→ `sem_imagem`); iOS pode mudar o workaround que a extensão usa para abrir o app (se falhar, o conteúdo fica no App Group e é lido no próximo foreground).

### Feature: Compartilhar Niks score (colagem + adesivo → Story)

> **Estado: VALIDADO no device (Sessão 58).** O fluxo inteiro foi visto rodando no iPhone físico: captura das 4 fotos, colagem no preview e adesivo. Restam **dois** pontos nunca conferidos — o **arquivo exportado** e o **espelhamento da foto frontal** —, marcados com 🔍 abaixo.
...
- **`share-capture.tsx`** — monta a colagem. **Réplica da tela "Layout" da câmera de Stories do Instagram** (referência dada pelo usuário na Sessão 57). Fundo **PRETO** (`<StatusBar style="light" />`), quadro 9:16 **da largura cheia da tela**, começando em **`top: insets.top`** (encostado no limite da área segura, logo abaixo do relógio — como o Instagram) e **sem cantos arredondados**, com os controles flutuando por cima.

> ⚠️ **A LARGURA MANDA — não voltar a limitar o quadro pela altura.** Ele já foi `Math.min(width, availH * 9/16)`, e o que sobrava de largura virava **faixa preta nas laterais**; o usuário reclamou disso **duas vezes** ("as grades têm de cobrir a tela toda, como no Instagram"). Hoje `FRAME_W = width` e a altura sai de 16/9. Corolários que fazem parte da mesma decisão: **`borderRadius: 0`** (canto arredondado É uma borda visível contra o preto) e a **barra de controles FLUTUA** (`position: absolute`) — em telas curtas, onde 16/9 da largura não cabe na altura livre, o disparo cobre a base da grade em vez de espremê-la, exatamente como o Instagram faz. Pôr a barra no fluxo encolheria a grade de novo.
>
> ⚠️ **`top: insets.top`, não `0`.** Colado em `0` a grade passa **por baixo da status bar** e o relógio fica em cima da imagem — foi assim numa rodada e o usuário pediu para respeitar o limite do topo, "mas literalmente no limite". Com isso o `X` de fechar volta a ser `top: 12 * S` **relativo ao quadro** (sem somar `insets.top`).
>
> ⚠️ **A ALTURA cede, a LARGURA não.** `FRAME_H = min(width * 16/9, height − insets.top − BOTTOM_BLOCK)`. Com 16/9 puro a grade encostava no disparo (reclamação do usuário); como a largura é intocável, quem cede é a **proporção**. O desvio fica em ~2%, some no `cover` das células e é muito menos perceptível do que a faixa preta lateral que voltaria se a largura cedesse junto para manter 16/9 exato.
>
> ⚠️ **Medidas explícitas, não "centraliza e torce".** `BOTTOM_BLOCK = GAP_ABOVE + SHUTTER + max(14 * S, insets.bottom)` é declarado **antes** da geometria, porque é ele que diz quanta altura a grade cede. O respiro entre grade e disparo é `GAP_ABOVE` — um número para ajustar, não sobra de arredondamento.
>
> ⚠️ **O "virar câmera torto" tinha causa de layout, não de estilo.** Ele é `position: absolute` e, solto numa caixa com `justifyContent: 'center'` + `paddingBottom`, quem definia a altura dele era o pai — que o alinhava pelo box **inteiro** enquanto o disparo era centrado no box **menos o padding**. Centros a ~15pt de diferença. A correção é estrutural: os dois vivem numa **linha de altura `SHUTTER`** com `alignItems: 'center'`, e aí os centros coincidem por construção. Não "consertar" isso com um `top` mágico no botão. ⚠️ **O fundo branco já foi tentado e REPROVADO no device** — ele revela os cantos arredondados e as sobras do quadro 9:16, e a tela deixa de parecer a do Instagram (onde a grade "cobre a tela"). No preto essas sobras somem. **Não trocar por branco de novo.** ⚠️ **Não existe mais header "Sua colagem", fundo branco, botão "Continuar", contador nem legenda** — foram removidos a pedido do usuário; não reintroduzir.
  - **UMA `CameraView` na CÉLULA DA VEZ**, sempre montada e **reposicionada** sobre a célula ativa (decisão 26); grade cheia → `active={false}` pausa sem desmontar. ⚠️ **Aqui NÃO seguimos o Instagram, e é deliberado.** A versão de **quadro inteiro** (câmera cobrindo a grade toda, foto entrando na célula em `cover`) chegou a ser construída e foi **REPROVADA pelo usuário no device**: com a câmera cobrindo tudo, a célula ativa vira um **recorte** da cena e a pessoa tem de **encaixar o rosto naquele quadrante** — mexer a cabeça, o celular, o ângulo. Com a câmera na célula, o quadrado **é** o enquadramento: o que ela vê no quadrado é o que cai no quadrado. **Não "consertar" isto de volta para o modelo do Instagram.**
  - Começa na **frontal**; o lado é estado (`facing`). ⚠️ **`mirror` só na frontal** (`mirror={facing === 'front'}`, ver decisão 27): espelha a FOTO DE SAÍDA para bater com o preview, que o iOS já espelha na frontal — na traseira o preview NÃO é espelhado, e manter `mirror` inverteria a cena de verdade.
  - **A célula ativa é DERIVADA, não estado:** `photos.findIndex(p => p == null)`. O `activeIndex` em `useState` foi removido — era ele que precisava ser reempurrado a cada foto e a cada "refazer", e que podia dessincronizar. Derivando, "refazer a célula 1 com as outras cheias" traz a câmera de volta pra ela sem nenhuma linha extra.
  - **Células vazias = a câmera AO VIVO, ampliada e borrada** — o efeito do Instagram, via o módulo nativo `modules/niks-camera-mirror` (ver abaixo). Ele desenha o fundo cobrindo o quadro inteiro; a célula da vez (câmera nítida) e as cheias (fotos) são pintadas **por cima**, então o borrão só sobra nas vazias. O escurecimento é a prop `dim` (hoje `0.22`), calibrável **do JS, sem build nativo novo**.
  - ⚠️ **Degradação obrigatória:** `mirrorOn = cameraReady && cameraMirrorAvailable`. Sem o módulo no binário (Android, ou JS novo sobre build antigo) as células vazias voltam ao **placeholder escuro `#1B1B1D` com ícone `Camera`** nas que não são a da vez. Esse caminho é o fallback, não código morto — **não remover**.
  - ⚠️ **Divisórias SUTIS, nunca brancas sólidas** — `GRID_LINE = rgba(255,255,255,0.55)` numa goteira de `FRAME_W * COLLAGE_GAP_FRACTION`. É cópia da linha discreta do Instagram, **pedida explicitamente** depois de a versão sólida deixar a grade "riscada". Não voltar para branco sólido.
  - **Refazer:** um toque numa célula cheia limpa a foto (sem menu/confirmação).
  - ⚠️ **Controle "Alterar grade" = glifo PELADO + rótulo, no canto inferior esquerdo da PRIMEIRA CÉLULA** — não no canto do quadro. É cópia do controle do Instagram, pedida assim ("mesmo símbolo, mesma posição, com o texto do lado"): sem círculo de fundo, com o texto **"Alterar grade"** ao lado. Numa grade 2×2 isso cai no meio da tela, à esquerda, igual à referência. **Não reintroduzir o círculo nem mover para o canto do quadro.** O glifo é **dinâmico** (`GridIcon` desenha a grade escolhida) enquanto o do Instagram é fixo — assim o controle também informa em que grade ela está. Altura 44 = alvo de toque mínimo da Apple; o único feedback de "bandeja aberta" é a opacidade.
  - **A bandeja abre ABAIXO do controle** (como no Instagram) e **vira para cima** quando não cabe — caso real das grades cuja primeira célula chega perto da base (`full`, `cols2`).
  - **Controles:** `X` de fechar (canto sup. esq., sobre a câmera) · botão da **grade** (canto inf. esq.) · **disparo** centrado na barra + **virar a câmera** à direita, na mesma linha — posição do Instagram e o "ao lado do disparo" que o usuário pediu. ⚠️ O ícone de virar é **`RefreshCw`** (as duas setas em laço do Instagram), **não** o `SwitchCamera` do lucide (câmera com setas) que havia antes. Sobre o preto: círculo `rgba(255,255,255,0.14)` com ícone branco. ⚠️ O **disparo é BRANCO** (anel branco + fresta escura + miolo branco) — o botão padrão do iPhone, que todo mundo reconhece. Já foi coral e o usuário pediu para trocar; **não repintar de coral**. A seta de "seguir" (grade cheia) é `INK`, senão some no miolo branco.
  - **Avanço sem "Continuar":** grade cheia → navega sozinha (`advance()` grava `collagePhotos` **e `collageGrid`** no store). ⚠️ `advancedRef` impede o disparo repetido quando a usuária **volta** do preview com a grade ainda cheia (seria impossível sair da tela); ele rearma quando a grade deixa de estar cheia. E, com a grade cheia, o **próprio disparo vira uma seta** (`ArrowRight`) — é a única saída de quem voltou, e não é um "Continuar" novo: mesmo botão, mesmo lugar.
  - **Galeria: só no modo degradado.** Não há mais ícone de galeria por célula. Quando **não há câmera ao vivo** (simulador ou permissão negada de vez), o **disparo** cai em `handlePick` e a mensagem diz isso. É o que impede a tela de virar beco sem saída.
  - Fotos reduzidas a **1080px no maior lado** (`expo-image-manipulator`) — várias fotos em resolução cheia estouram a memória.
  - `capturingRef` como mutex (decisão 25) e o índice-alvo é **congelado antes do `await`**: sem isso a célula ativa muda durante a captura e a foto cai na errada.

#### O fundo borrado ao vivo — `modules/niks-camera-mirror` (módulo NATIVO)

> ⚠️ **Este é o único código nativo escrito à mão do projeto.** Mexer nele exige `pod install` + **build nativo novo** — recarregar o app não basta.

**O problema.** No iOS, **uma `AVCaptureSession` pode alimentar VÁRIAS `AVCaptureVideoPreviewLayer`**. É assim que o Instagram mostra a câmera **nítida encaixada na célula da vez** e, ao mesmo tempo, **ampliada e borrada** no resto da grade: uma câmera, duas exibições. Só que o **`expo-camera` cria uma sessão NOVA para cada `<CameraView>`** (`CameraSessionManager`, `let session = AVCaptureSession()`, instanciado por view em `CameraView.swift`), e o **iOS só deixa UMA sessão segurar a câmera** — dois `<CameraView>` e a segunda derruba a primeira (tela preta). Nenhuma das ~19 props do `CameraView` resolve isso. Por isso o efeito é **impossível só com JS**.

> ⚠️⚠️⚠️ **NÃO EXISTE SEGUNDA PRÉVIA — as duas tentativas falharam no device.** Está aqui porque as duas parecem óbvias e alguém vai tentar de novo:
>
> 1. `AVCaptureVideoPreviewLayer(session:)` para a segunda camada **rouba a conexão da primeira** — esse init cria a conexão automática da porta de vídeo para a camada.
> 2. `init(sessionWithNoConnection:)` + `AVCaptureConnection(inputPort:videoPreviewLayer:)` à mão **também não resolve**: uma `AVCaptureSession` comum sustenta **UMA ÚNICA conexão de prévia**. O `addConnection` é **aceito** (`canAddConnection` devolve `true`) e a prévia do `expo-camera` **para de receber quadros mesmo assim**.
>
> **Sintoma idêntico nos dois casos:** a célula da vez fica **transparente** e a tela inteira vira o borrão — o que se vê ali é o fundo do espelho, através dela.

**A solução.** Uma `ExpoView` que **não abre sessão nem disputa a prévia**: ela acha a sessão que já roda e pendura nela um **`AVCaptureVideoDataOutput`** — canal SEPARADO (o mesmo que apps de filtro usam), que convive com a prévia e com o `AVCapturePhotoOutput` do disparo. O fundo é desenhado a partir dos quadros crus, numa `CALayer` com `contentsGravity = .resizeAspectFill`, sob `UIVisualEffectView(.dark)` + um véu de alpha ajustável.

> **O borrão é de graça:** cada quadro é reduzido a **~80 px de largura** e depois esticado pela camada — **a perda de resolução É o desfoque**, não há filtro caro por quadro. Uma gaussiana de `sigma 1.4` entra só para o esticamento não sair quadriculado (com `clampedToExtent`, senão a gaussiana escurece as bordas). Throttle de **~12 fps**: é fundo borrado, mais que isso é bateria à toa. E `CATransaction` com ações desligadas ao trocar `contents`, senão o cross-fade padrão do CALayer deixa rastro.

- ⚠️ **Como acha a sessão:** varrendo a árvore de views atrás de uma `AVCaptureVideoPreviewLayer` já montada e lendo o **`.session`** dela. **De propósito NÃO importamos o `ExpoCamera` nem lemos campos privados por reflexão** — `AVCaptureVideoPreviewLayer.session` é API pública da Apple e não quebra quando o expo-camera muda por dentro. A busca **ignora a própria subárvore** (senão o espelho se acha a si mesmo).
- ⚠️ **A busca REAGENDA** (60 tentativas × 150 ms): a prévia do expo-camera só é inserida no `layoutSubviews` dele, que pode acontecer **depois** do nosso mount. Desistir na primeira tentativa dá fundo preto intermitente.
- ⚠️⚠️ **NÃO TOCAR NA CONFIGURAÇÃO ENQUANTO `session.isRunning == false`** — outro bug já visto no device. `beginConfiguration`/`commitConfiguration` numa sessão que o expo-camera ainda está montando (na fila DELE, enquanto o espelho está na SUA) é **configuração concorrente sobre o mesmo objeto**. **Sintoma: ~10 segundos de tela preta** ao entrar na tela, antes de a câmera abrir — o espelho reconfigurava a sessão a cada 120 ms enquanto ela tentava subir. Esperar `isRunning` custa nada. Pelo mesmo motivo, recusa de conexão e re-vínculo pós-troca-de-câmera usam **backoff de 0,5 s**, não o retry curto.
- ⚠️ **`previewReady` (JS) faz par com isso.** A sessão leva um instante para subir; sem esse estado a tela fica um **retângulo preto** nesse intervalo (sem câmera e sem placeholders, porque o `mirrorOn` já estava ligado). `mirrorOn = cameraReady && cameraMirrorAvailable && previewReady`, com `previewReady` vindo do `onCameraReady` do `<CameraView>`.
- ⚠️ **Troca frontal↔traseira reconfigura a sessão, e há um WATCHDOG para isso.** O `session` do expo-camera é `let` e nunca é recriado, mas `updateDevice()` mexe nos inputs e o nosso output pode sair junto — o fundo congelaria. Um `Timer` de 0,5 s checa `session.outputs.contains(output)` e se pendura de novo; enquanto está tudo certo, só reafirma orientação e espelhamento (barato, não mexe na configuração da sessão).
- **Se a sessão RECUSAR o output** (`canAddOutput == false`), nada é quebrado — a prévia do expo-camera fica intacta — e a tela degrada para um fundo escuro liso (a `backgroundColor` da view nativa é o mesmo `#1B1B1D` do placeholder do JS, de propósito).
- **Conexão fixada em `.portrait`** (o app é travado em retrato no `app.json`): sem isso a segunda prévia nasce deitada em alguns aparelhos.
- **`isUserInteractionEnabled = false`** — é decorativo; o toque tem de continuar chegando na grade.
- **JS:** `cameraMirrorAvailable` usa `requireOptionalNativeModule` e é o que **impede o app de quebrar** para quem estiver com o binário antigo e o JS novo. Nunca renderizar a view sem checar essa flag.
- **Só iOS** (`expo-module.config.json` → `"platforms": ["apple"]`). No Android a flag é `false` e a tela cai no placeholder escuro.

#### A grade é ESCOLHIDA, não deduzida — `lib/collageGrid.ts`

**FONTE ÚNICA do layout da colagem**, consumida pela captura (grade ao vivo sobre a câmera), pelo `<Collage>` do preview e pela imagem exportada. Antes o layout era **deduzido da quantidade de fotos** — o que tornava impossível pedir "duas lado a lado", porque 2 fotos sempre viravam faixas empilhadas.

| id | arranjo | células |
|---|---|---|
| `grid4` | 2×2 | 4 |
| `rows2` | duas faixas empilhadas | 2 |
| `rows3` | três faixas empilhadas | 3 |
| `top1bottom2` | uma larga em cima + duas embaixo | 3 |
| `full` | foto única | 1 |
| `cols2` | duas colunas lado a lado | 2 |

- A **ORDEM do array é a da bandeja** (2 colunas × 3 linhas, igual ao Instagram). As quatro primeiras caem nas MESMAS posições da bandeja do Instagram; no lugar da grade de **6 células** (que a colagem final não desenha) entrou a **foto única**, pedida explicitamente pelo usuário.
- ⚠️⚠️ **`COLLAGE_GAP_FRACTION` é ZERO — as fotos se TOCAM na colagem final, e isso é intencional.** A colagem do Instagram usada como referência **não tem goteira**: a "divisão" entre as fotos é só a troca de imagem. Qualquer valor > 0 vira **linha branca** no arquivo exportado, e o usuário reclamou disso **três vezes** — a última já com a goteira afinada ("não quero que tenha essa linha branca no meio"). **Não pôr valor aqui.**
- ⚠️ **A captura NÃO depende dessa fração para mostrar a grade.** Lá as divisórias são **linhas de `GRID_LINE_W` pt traçadas POR CIMA das células**, centradas na fronteira — servem para enquadrar e **não deixam rastro na imagem exportada**. É esse desacoplamento que permite goteira zero no resultado E grade visível na hora da foto. Não "consertar" a grade da captura pondo goteira na colagem.
- A fração existe (em vez de pontos) porque a captura (largura da tela), o preview (~293pt) e a imagem exportada (360pt) desenham a MESMA grade em **três escalas**; em pontos, a goteira de `2 * S` do preview virava **~8px no arquivo de 1080px**. Se um dia a goteira voltar, é o **único** número a mexer — `0.003` ≈ 3px em 1080.
- **`cellRects(grid, W, H, gap)`** devolve o retângulo de cada célula. ⚠️ Os números são EXATAMENTE os que o flex calcula no `<Collage>` (linhas `flex: 1` sobre o que sobra das goteiras) — é essa igualdade que deixa a captura sobrepor fotos e divisórias a uma câmera de quadro inteiro e ainda mostrar a grade que será exportada.
- **`gridForCount(n)`** é o mapa histórico (1 → inteira · 2 → empilhadas · 3 → uma em cima + duas · 4 → 2×2). Virou **rede de segurança** do preview: se o hand-off do store trouxer uma grade que não bate com a quantidade de fotos (app reaberto no meio do fluxo), a colagem cai nele em vez de desenhar células vazias.
- **`components/share/GridIcon.tsx`** desenha o glifo de qualquer grade **a partir do próprio `cellRects`** (Views, não asset nem lucide): grade nova em `COLLAGE_GRIDS` ganha ícone de graça, e o ícone nunca pode divergir da grade que ele representa.
- **Store:** `collageGrid` viaja junto de `collagePhotos`, **fora do `partialize`** (mesmo motivo das fotos: hand-off de tela, de vida curta). Sem ele a colagem final não sabe se as 2 fotos são empilhadas ou lado a lado.
- **Trocar a grade preserva as fotos que couberem**, na ordem; o que não cabe é descartado e isso fica visível na hora (as células somem).

- **`share-preview.tsx`** — colagem final + adesivo + exportação.
  - **Layout vem da GRADE escolhida** (`collageGrid` no store), não da quantidade de fotos — ver `lib/collageGrid.ts` acima. Sempre `cover` (nunca esticada). Fallback em `gridForCount` quando o hand-off não bate.
  - ⚠️⚠️ **Células ABSOLUTAS (`cellRects`), nunca linhas de `flex: 1` — e com SANGRIA.** Este é o desfecho da "linha branca entre as fotos", reportada **três vezes** e que **sobreviveu à goteira zero** porque nunca foi goteira: com `flex: 1` o Yoga arredonda cada célula para o pixel e entre duas vizinhas sobra uma **fresta sub-pixel** por onde o fundo aparece — e o fundo era **branco**. Correções, todas necessárias juntas: (1) células posicionadas por `cellRects`, o MESMO helper da tela de captura; (2) **`bleed` de 0,5pt** por lado quando a goteira é zero, fazendo as vizinhas se **sobreporem** para que nenhum arredondamento abra fresta (com goteira > 0 a sangria é 0, senão ela comeria a goteira); (3) fundo do container **só branco quando há goteira de propósito**, preto caso contrário. **Não voltar para linhas `flex: 1`.** A sangria é de **1pt** (não 0,5): no @3x meio ponto ainda arredonda para 1px de cada lado, e a sobreposição precisa sobreviver ao arredondamento nas **três** escalas em que a colagem é desenhada.
  - ⚠️ **A foto É a célula — mesma estrutura da tela de captura.** `<Image>` posicionada direto no retângulo de `cellRects`, **sem View-embrulho, sem `overflow: 'hidden'` e sem tamanho em `'100%'`**. Era essa a diferença que fazia a captura sair sem emenda e a colagem sair com "linha branca": o embrulho somava dois arredondamentos a mais (a caixa e a porcentagem) e ainda **recortava** a imagem numa fronteira fracionária, deixando uma fileira de pixels meio transparentes. `resizeMode="cover"` já recorta dentro do próprio frame — o embrulho nunca foi necessário. **Manter as duas telas com a MESMA estrutura**; divergir aqui foi o que custou várias rodadas.
  - ⚠️ **NÃO existe mais nenhuma superfície branca atrás da colagem** — nem o container do `<Collage>`, nem o wrapper do preview (ambos `#000`). Isso é diagnóstico, não estética: se uma linha voltar a aparecer entre as fotos, **clara = alguém reintroduziu um branco no caminho; escura = antialias de borda de imagem** (aí a correção é a sangria, não a cor). Os brancos que sobraram na tela (fundo da página, botão voltar, pílula "Trocar adesivo", "Salvar na galeria") ficam todos FORA da colagem.
...
1. **Espelhamento da foto frontal.** Pela leitura do source nativo, `mirror` deve fazer a foto salva bater com o preview (decisão 27) — mas isso **nunca foi testado com um objeto assimétrico/texto**. Se sair espelhado, o ajuste é um `flip: Horizontal` no `expo-image-manipulator`. ⚠️ Hoje `mirror` é **condicional** (`facing === 'front'`); na traseira ele tem de ficar desligado.
...
3. ~~`share-preview` inteira nunca rodou no device.~~ **Resolvido na Sessão 58** — a tela foi vista rodando, com colagem e adesivo.
...
2. **Suas preocupações** — condicional (`concerns_alignment`): card com borda esquerda colorida (verde=confirmado / âmbar=parcial / coral=divergente), badge + mensagem de 2 frases que **conecta os `concerns` declarados no onboarding com os achados do scan** ("Suas preocupações fazem sentido: a análise confirma…"). Era o antigo "Objetivo Validado" (`goal_alignment`, sobre `objetivo`), repurposado para concerns na Sessão 55
...
**CTA flutuante "Seguir para a minha rotina" → `/protocolo`.** Mesmo padrão do CTA do resultado do **onboarding** (`(scan)/results.tsx`, decisão 5): `Animated.View` `position:'absolute'` **irmão do `Animated.ScrollView`** (fora dele), que **surge com fade + slide-up** quando o topo da seção "O que fazer pela sua pele" alcança o meio da tela (`onLayout` daquela View grava `sectionY`; gatilho = `HERO_HEIGHT + sectionY − viewportH*0.5`). Fallback para resultado **sem** essa seção: `maxScroll * 0.4` — nunca deixar a usuária sem CTA. Um `scrollY.addListener` liga `floatActive`, que alterna `pointerEvents` `'none'`/`'box-none'` — sem isso o bloco roubaria o scroll da base da tela enquanto invisível. Visual: pílula `#FF9D9D`, `height 60`, radius 100, texto branco **Nunito ExtraBold 17** (`useFonts` local; `undefined` enquanto carrega, nunca `''`).
> ⚠️ **A navbar global cobre esta tela.** `skin-result` é `Tabs.Screen` com `href: null` — some da barra **como aba**, mas a `GlobalBottomBar` continua sendo desenhada por cima (ela é irmã do `<Tabs>` no `(app)/_layout.tsx`, e pela **decisão 18** nenhum zIndex de um filho vence o pai). Por isso o bloco tem `paddingBottom: NAVBAR_H + CTA_GAP` (80 + 20), como o "Escanear" da home — e **não** `insets.bottom`: a navbar já cobre o home indicator, e somar a safe area de novo abriria um vão grande sobre o menu (mesma armadilha do `ChatInputBar`). O `paddingBottom` do `contentContainerStyle` passou a somar `CTA_BLOCK` (navbar + respiro + botão + fade) para o último conteúdo rolar inteiro.
...
> ⚠️ **REDESENHADA (jul/2026) para a identidade "Novo design app NIKS"** — era a **última tela presa no primeiro design** (creme `#F6F4EE` + teal `#1D3A44` + coral antigo `#FB7B6B`, sem fonte definida), o que a fazia "parecer de outro app". Agora segue o mesmo sistema de home/protocolo/niks-chat: **inline styles + tokens locais** (não NativeWind, não `Colors`), fundo **branco `#FFFFFF`**, fonte **Nunito** (`useFonts` → `fXBold/fBold/fSemi/fMed`), ink `#121212`/`#515151`/`#818181`/`#B5B5B5`, rosa `#FF9D9D` (marca) / `#F2808E` (eyebrows/premium) / `rgba(255,157,157,0.14)` (wash), bordas `#E3E3E6`, hairlines `rgba(18,18,18,0.06)`. Vermelho `#D4183D` mantido como cor **semântica** de ação destrutiva (apagar/sair). O design Figma Make antigo (`cFsFcVSjOMkTdHIJpHgSDk`) foi aposentado.

Componentes locais: `IconChip` (quadrado 38×38 arredondado tintado de rosa — ou vermelho no destrutivo), `Row` (linha de config: chip + label + ChevronRight, com `hairline` opcional e flag `danger`), `SectionLabel` (eyebrow uppercase, `letterSpacing` 0.8, `INK_MUTE`), `CARD` (estilo compartilhado dos cards). Layout (de cima para baixo):
1. **Header** — logo `niks-logo.png` (sparkle) tintada de rosa + wordmark "NIKS", **centrado** (o mesmo padrão de header do niks-chat), com hairline embaixo.
2. **Card de perfil** (clicável → `set-name.tsx`):
   - Avatar 62×62 em `LinearGradient` coral (`['#FFB4B4','#FF9D9D']`) com a inicial do nome em branco (ou `?` se não definido)
   - Pílula "Premium" (fundo rosa translúcido + Crown `#F2808E`)
   - Nome do Supabase (`users.nome`) ou "Toque para definir" se vazio + subtítulo "Toque para editar seu perfil" + ChevronRight
3. **Seção "Assinatura"** — "Gerenciar assinatura" (chip Crown) → `Linking.openURL('itms-apps://apps.apple.com/account/subscriptions')`
4. **Seção "Seu e-mail"** — card com dois itens: e-mail (linha estática, chip Mail) + botão "Apagar minha conta" (chip/ícone `Trash2` vermelho). `onPress` exibe Alert de confirmação; confirmado → `deleteAccount()` + `router.replace('/')`
...
6. **Seção "Suporte"** — "Fale conosco" (Alert + `mailto:support@niksaiapp.com`) + "Avaliar o app" (`requestAppReview()`)
7. **Sair da conta** (botão pílula vermelho translúcido, `signOut` com Alert de confirmação) + versão lida de `Constants.expoConfig.version` ("NIKS v1.0.x")

**Dados dinâmicos (nome + e-mail):** carregados via **`useCachedQuery(`perfil:${uid}`)`** (`supabase.auth.getSession()` + `users.nome`, `staleMs` 5 min) — a tela abre preenchida e revalida em silêncio, **sem `getUser()` (ida à rede) por foco**. `useFocusEffect` apenas chama `setTabBarTheme('light')` (o protocolo pode deixar a navbar escura ao sair).
...
> **Com câmera trocável, passe `mirror` CONDICIONAL ao lado** (`mirror={facing === 'front'}` — é o que a `share-capture` faz). No iOS o guard `presetCamera == .front` do snippet acima já ignora `mirror` na traseira, mas a condição explícita é a que documenta a intenção e a que protege no Android — na traseira o preview **não** é espelhado, então espelhar a saída inverteria a cena de verdade (texto ao contrário).
...
*Sessão 66 — Setembro 2026 — **Minha Coleção: a rotina passa a usar os produtos que ela JÁ TEM EM CASA.** Antes, a IA montava a rotina ideal só com o catálogo e a usuária recebia uma lista de compras; agora o catálogo só preenche **o que falta ou não serve**. **(1) A regra que organiza tudo:** a ESTRUTURA da rotina continua vindo do scan (`generate-protocol`); a Coleção muda **só qual produto ocupa cada passo** — é isso que permite refazer a escolha sem reescrever a rotina, que precisa ficar estável ~30 dias. Logo, o lugar da mudança é a **`recomendar-produtos`**, não a `protocolos`. **(2) Tabela nova `colecao_produtos`, não uma flag em `product_scans`:** scan é histórico imutável, Coleção é estado atual mutável (ela remove, refaz foto, a compat é recalculada) — com uma flag, "remover da Coleção" apagaria o histórico; e produto **não identificado** nem chega a virar `product_scans`. É a primeira tabela de produto com **escrita do CLIENTE** (RLS própria); a porta única é `lib/colecao.ts`. **(3) O ponto não óbvio — a fila espera o scan:** no primeiro fluxo os produtos são fotografados ANTES do rosto, e a `analisar-produto` mede contra o ÚLTIMO scan; analisar na hora mediria contra o scan do onboarding (1 foto) e exigiria recalcular depois = **dobro de chamadas de visão por produto**. Enfileirar as fotos e analisar DEPOIS do scan de 6 fotos custa UMA chamada por produto e já nasce certo. **(4) Este fluxo ENGLOBOU o gatilho da regeneração única do 1º scan in-app** — `loading-dentro-app` chama `finalizarPrimeiroFluxo`, que envelopa o `regenerateProtocolInApp` de sempre (mesmos guards); não há segundo caminho de regeneração. **(5) `preservar_catalogo` na `recomendar-produtos`:** rechamar com `regenerate: true` re-rodava a IA para TODOS os passos e, como o modelo não é determinístico, **adicionar um produto trocaria em silêncio os recomendados de passos sem relação**. Agora passos intocados mantêm o que tinham — e, se nada sobrar, a chamada ao modelo nem acontece. **(6) Travas clínicas aplicadas ao PRODUTO dela**, não só ao passo: retinoide/AHA nunca ocupam passo de manhã (o passo pode ser um hidratante AM e o creme dela conter retinol), não dobrar família forte no mesmo período, gestante sem retinoide, e passo de Tratamento exige o ativo que ele pede. **(7) O marcador `users.colecao_onboarding_at` é gravado na RESPOSTA do pop-up, não no fim do fluxo**: todo scan de 6 fotos passa pelo `finalizarPrimeiroFluxo`, inclusive o de quem nunca viu o pop-up porque o gate falhou por rede — marcar lá esconderia o fluxo dessa pessoa para sempre. **(8) `lib/savedProducts.ts` APOSENTADO** (sobrou o `normStepKey`): o "Salvar na minha rotina" (AsyncStorage, por aparelho) virou **"Tenho esse produto em casa"** → Coleção no servidor, e a foto do passo na Rotina passou a vir de `lib/rotinaProdutos.ts`. Dados antigos não são migrados (não dá para deduzir deles que ela TEM o produto). **(9) Produto do catálogo declarado entra SEM número de compatibilidade** — a régua sai de uma análise de visão do produto real, e o projeto não inventa número: o card mostra "Recomendado pra você". **(10) Pop-up pós-scan mora no `product-result`** (os dois caminhos de scan terminam lá) e **não** no `ProductAnalysis`, que é compartilhado com o histórico — senão ele apareceria ao abrir um scan antigo. **(11) NIKS Chat** ganhou `<ProductsAtHome>`, sempre presente (inclusive afirmando coleção vazia — a mesma lição do `<PendingProtocolSuggestion>`). **Teto de 12 produtos por sessão** (custo de IA + abandono é o risco nº 1). **(12) Os DOIS botões foram separados por origem, e é a distinção que sustenta a feature:** produto vindo de **scan** → "Tenho esse produto em casa" (Coleção, foto dela, compatibilidade medida, vai pro chat); produto vindo do **catálogo** (detalhe de um recomendado **e também o da alternativa**, que antes não tinha botão nenhum) → **"Adicionar à minha rotina"**, que FIXA aquele produto naquele passo (`rotina_produto_fixado`) e **não** entra na Coleção — afirmar que ela tem em casa um produto que ela só escolheu seria mentira, e é a Coleção que alimenta o "o que eu faço agora?" do chat. **(13) A escolha dela ganha da IA:** a ordem de resolução virou `fixado` → `em_casa` → catálogo, então um produto de casa compatível que apareça depois **não derruba** o que ela fixou; o fixado é revalidado nos gates clínicos antes de ser aplicado (gravidez/alergia nova/produto fora do catálogo → cai no fluxo normal). **(14) O "texto duplicado" no passo da Rotina NÃO era bug de render** — `mapStep` não concatena nada; o prompt do `generate-protocol` manda repetir o TIPO do produto em `name` e `ingredient` de propósito. O subtítulo passou a mostrar **marca + nome do produto** quando o passo tem produto escolhido (e só cai no `ingredient` quando não tem). ⚠️ Fica um achado **de dados** em aberto: `ingredient` com MARCA dentro ("Gel de Limpeza La Roche-Posay Effaclar Gel") viola o spec do campo — ver o callout "Subtítulo do passo" na seção da Rotina. **(15) MARCA no campo `ingredient`, consertado nas duas pontas** (bug confirmado no banco: `ingredient` = "Gel de Limpeza La Roche-Posay Effaclar Gel"). Detector único `marcaComercialEm()` em `_shared/protocol-write.ts` (lista explícita, casamento por palavra inteira, 14 casos de teste incluindo os falso-positivos óbvios — "Sérum com Centella Asiática (Cica)" não casa); `generate-protocol` ganhou a **Checagem 3** na camada de validação que já existia (prompt sozinho já tinha falhado) + o spec de `ingredient`/`name` proibindo marca; `validateProposal` **recusa** a proposta do Coach em vez de sanitizar (limpar a marca ainda deixaria "Effaclar Gel"). ⚠️ **Sem backfill** — linhas já gravadas continuam tortas até serem regeneradas. **(16) Compatibilidade 0–100 nos produtos do CATÁLOGO, na mesma régua da Coleção.** A Camada 1 do prompt clínico foi extraída para `_shared/compatibilidade.ts` (a `analisar-produto` passou a interpolá-la — prompt em runtime **byte a byte idêntico**, conferido), e a `recomendar-produtos` a reusa numa **chamada em lote** (~2,7k tokens; produto a produto seriam ~61k). ⚠️ **Pontua DEPOIS de responder** (`EdgeRuntime.waitUntil`): a tela recebe a recomendação na hora e as notas pousam em seguida — a tela repesca a cada 5 s por até ~30 s. **Zero latência a mais no `generateOnDemand`**, que era a preocupação. Só pontua produto sem nota → fixar produto não dispara chamada. **Decisão de produto: a diferença de evidência (foto × ficha) NÃO é sinalizada na interface** — só planta dúvida sobre o número; fica registrada no código e aqui. ⏳ **Deploy pendente:** migrations `20260922120000` e `20260922130000`; `recomendar-produtos`, `niks-chat`, `generate-protocol`, `approve-coach-protocol-change` **e `analisar-produto`** (o prompt dela agora importa de `_shared`) (estas duas últimas porque `_shared/protocol-write.ts` mudou). ⚠️ **Subir `generate-protocol` leva junto as 4 edições de prompt pendentes desde a Sessão 57** — o deploy é do arquivo inteiro, não dá para escolher. **Nada testado em device**. Ver "Feature: Minha Coleção".*
...
*Sessão 59 — Setembro 2026 — **Feature "Compartilhar com o NIKS" (Share Extension iOS).** Do Mercado Livre, Safari ou site de marca, Compartilhar → NIKS abre o app numa tela de carregamento nova (`share-product-loading`: anel girando + crossfade rosto↔produto + frases) e cai no `product-result` de sempre, salvo em `product_scans`. **(1)** `expo-share-intent` 6.1.1 — extensão nativa leve (target `NIKSShare`, bundle `br.com.niksai.app.share-extension`, App Group `group.br.com.niksai.app`); patch local p/ `iosShareExtensionDisplayName` (o target não pode se chamar `NIKS`). **(2)** Nova Edge Function **`extrair-imagem-produto`** (og:image/JSON-LD/twitter:image + fallback API do Mercado Livre, proteção SSRF) — **deploy pendente**; `analisar-produto` aceita `productName` opcional (contrato mantido) — **deploy pendente**. **(3)** Consumo do share SÓ no `(app)/_layout` depois do guard (o grupo `(scan)` não tem guard) → respeita sessão/paywall. **(4)** Lógica do loading de produto extraída para `hooks/useProductAnalysis` (ganhou guard de execução dupla, flag de desmontagem e Mixpanel `product_scan_*` com `origem`). **(5)** `useScanConsentGate` passou a expor `granted` e aceitar `onDecline`. **(6)** README corrigido: a pasta nativa é `ios/NIKS/`, não `ios/NIKSAI/`; `ios/Podfile` ganhou os `extraPods` com `modular_headers` que não estavam sendo aplicados. Ver "Feature: Compartilhar com o NIKS".* **Validado no simulador (ponta a ponta):** Lembretes (texto com link do ML) e Safari (página) → tela de carregamento nova → `product-result` → linha em `product_scans` com a foto 512×512. **Peças extras desta sessão:** WebView invisível (`HiddenPageImageReader`, 25 s) para links do ML que o backend não lê — achou a foto em ~1,8 s; foto do share **normalizada para quadrado com fundo branco** (Skia offscreen — o `extent` do ImageManipulator é só web); consentimento **inline** na tela (o `<Modal>` nativo não aparece se houver seletor de fotos apresentado — travava a tela em 0%); `lib/shareResume.ts` (a tela sobrevive a remount); `lib/nativePresentation.ts` + `dismissAll` no (app)/_layout para nada ficar por cima; 2º patch do plugin (App Group duplicava a cada prebuild); leitor da Amazon + `pagina_bloqueada`; regra do `productName` (ativos só do que está legível na foto). ⚠️ **Nada disso foi validado em iPhone** — falta App Group/assinatura reais, TestFlight e o app do ML de verdade.

*Sessão 58 — Agosto 2026 — **`share-capture` virou réplica da tela "Layout" do Instagram + a grade passou a ser ESCOLHÍVEL + primeiro módulo NATIVO do projeto.** Tudo a pedido do usuário, com prints do Instagram como referência. **(1) A tela foi reescrita:** fundo preto, quadro 9:16 **da largura cheia** começando em `insets.top`, sem cantos arredondados, controles flutuando por cima; **saíram** header "Sua colagem", fundo branco, botão "Continuar", contador e o ícone de galeria por célula (a galeria só sobrevive no modo degradado — simulador/permissão negada). Grade cheia → **avança sozinha**; quem volta do preview acha o disparo virado em **seta** (`advancedRef` impede o disparo repetido). Célula ativa virou **derivada** (`findIndex`), não estado. **(2) Nova `lib/collageGrid.ts` — FONTE ÚNICA do layout**, com 6 grades (`grid4`/`rows2`/`rows3`/`top1bottom2`/`full`/`cols2`), `cellRects` e `gridForCount` (o mapa antigo virou rede de segurança). A escolha viaja no store (`collageGrid`, fora do `partialize`) até o `<Collage>` do preview E da imagem exportada — antes o layout era **deduzido da quantidade de fotos**, o que tornava impossível pedir "duas lado a lado". Ícones por `components/share/GridIcon.tsx`, desenhados a partir do próprio `cellRects`. **(3) `modules/niks-camera-mirror` — o PRIMEIRO código nativo escrito à mão do projeto** (Swift, só iOS): as células vazias mostram a câmera **ao vivo e borrada**, como no Instagram. **Duas tentativas falharam antes de acertar:** `AVCaptureVideoPreviewLayer(session:)` rouba a conexão da prévia do `expo-camera`, e `init(sessionWithNoConnection:)` + `addConnection` à mão **também** (uma `AVCaptureSession` comum sustenta UMA conexão de prévia; o `addConnection` é aceito e a prévia original para mesmo assim). A solução é **`AVCaptureVideoDataOutput`** — canal separado — com o quadro reduzido a ~80px (a perda de resolução É o desfoque) a ~12fps. Mais dois bugs vistos no device e corrigidos: **~10s de tela preta** (configurar a sessão em paralelo com o expo-camera → hoje espera `isRunning`) e **fundo congelado ao virar a câmera** (watchdog de 0,5s). `cameraMirrorAvailable` impede o app de quebrar com JS novo sobre binário antigo. **(4) A "linha branca entre as fotos"** — reportada 4×, e **não era goteira**: com `flex: 1` o Yoga abria fresta sub-pixel entre as células e o fundo **branco** aparecia; o embrulho com `overflow: 'hidden'` em volta de cada foto ainda recortava numa fronteira fracionária. Correções: `COLLAGE_GAP_FRACTION = 0` (as fotos se TOCAM), células absolutas por `cellRects` com **sangria de 1pt**, `<Image>` **direto no retângulo** (mesma estrutura da captura, sem embrulho) e **nenhuma superfície branca atrás da colagem**. ⚠️ Boa parte das rodadas finais foi **cache do Metro** servindo bundle velho — o rebuild resolveu. **(5) `mirror` virou condicional** (`facing === 'front'`) porque a câmera passou a ser trocável. Ver "Feature: Compartilhar Niks score" (seções novas "A grade é ESCOLHIDA" e "O fundo borrado ao vivo") e a decisão 27.*

*Sessão 57 — Agosto 2026 — **(1) 1º scan in-app regenera o protocolo + (2) validação no `generate-protocol` + (3) fix do "pendente fantasma" no `niks-chat`.** **(1)** O PRIMEIRO scan feito DENTRO do app passa a regenerar a rotina **uma única vez, em background** (a usuária não espera), com **card-promessa** na `skin-result`; marcador `users.inapp_protocol_regenerated_at` gravado **só no sucesso confirmado** (read-back compara a linha de `protocolos` antes×depois — a do signup sempre existe), `recomendar-produtos` refrescada via `regenerate: true`, ordem tabela→store→cache. Reusa `generateAndSaveProtocol` + `lib/buildOnboardingDataFromUserRow.ts` (extraído do `generateOnDemand`); vive em `lib/regenerateProtocolInApp.ts` (função de módulo → sobrevive à navegação). Ver o callout "O PRIMEIRO scan pelo app regenera…" e o schema de `users`. **(2)** `generate-protocol` ganhou **validação determinística pós-IA** (ativo declarado vira PASSO **por molécula** + sem dois ativos da mesma classe no mesmo período; 1 retentativa corretiva) e **reforço no prompt** (regra de família de ativos) — corrige "declarou adapaleno, recebeu retinol". Ver a nota nova em "Edge Functions". **(3)** `niks-chat`: quando NÃO há sugestão pendente, o context pack passa a **afirmar** isso em `<PendingProtocolSuggestion>` (antes era omitido) — a NIKS lia a própria fala anterior no histórico e repetia "já tem uma sugestão aguardando", travando pedidos legítimos; a instrução deixou claro que o estado vem SEMPRE do contexto atual, nunca do histórico (as duas linhas que testavam "vazio"/"tem conteúdo" foram ajustadas p/ não bloquear toda proposta). **Deploy pendente:** migration `20260801120000` + `recomendar-produtos` + `niks-chat` + as 4 edições de prompt do `generate-protocol` (a camada de validação em si já está no ar).*

*Sessão 56 — Julho 2026 — **Preview "antes/depois" que travava + skeleton no cold start da home.** Dois problemas separados. **(1) `generate-skin-preview` travava por minutos / nunca gerava.** Causa: a chamada à OpenAI (`gpt-image-2`) não passava `quality` (padrão pesado, 1–2 min), não tinha timeout nem retry, e a `plan-preview` girava o spinner "pra sempre". Correção (deployada): Edge Function com **`quality: 'medium'` + timeout de 90s** (`AbortController` → 504); `loading.tsx` (onboarding) chama com **timeout de 100s + 3 tentativas**; `plan-preview.tsx` com **fallback em 45s** (cai na foto do próprio usuário). ⚠️ **Decisão:** a preview é EXCLUSIVA do onboarding — **removida a chamada da `loading-dentro-app.tsx`** (scan dentro do app NUNCA deve gerá-la). **(2) Flash de tela vazia na home (~3s no cold start).** `homeData` fica `null` enquanto sessão + cache do disco + rede resolvem, e a home mostrava os placeholders vazios (score "—", barras vazias). Novo **`components/Skeleton.tsx`** (bloco cinza pulsante) preenche score/foto/números das métricas enquanto `homeData == null && state !== 'error'`; some quando os dados chegam. Distinção mantida: quem nunca escaneou vê o placeholder legítimo (rostinho + "faça seu primeiro scan"), não o skeleton (gate é `homeData == null`, não `skinScore == null`). Ver a tabela de Edge Functions (`generate-skin-preview`) e "CACHE DE DADOS" (skeleton).*

*Última atualização: Sessão 55 — Julho 2026 — **O card "Suas preocupações" (antigo "Seu Objetivo") voltou à tela de resultado, agora baseado no que mais incomoda a usuária.** A pedido do usuário: o card `goal_alignment` — removido na Sessão 54 junto com o `objetivo` — foi **reaproveitado e renomeado para `concerns_alignment`**, com a mesma estrutura (badge confirmado/parcial/divergente + borda colorida + mensagem de 2 frases), só que a IA agora conecta os **`concerns`** (tela "O que mais te incomoda") aos achados do scan em vez do objetivo. Reintroduzido o bloco de prompt + a entrada no JSON de saída em **`analyze-skin` e `analyze-skin-app`** (ambas já recebiam `concerns`); no app, o tipo `ScanResult.goal_alignment` virou `concerns_alignment` e o render em `results.tsx`/`skin-result.tsx` passou a lê-lo (rótulo "Seu Objetivo" → "Suas preocupações"; badges "Alinha/Parcialmente alinhado" → "Confirmado/Parcialmente confirmado pela análise"). A mensagem começa por "Suas preocupações fazem sentido…". Scans antigos (com `goal_alignment`) simplesmente não mostram o card. Ver a tabela de Edge Functions, o tipo `ScanResult` e a tela de resultado do scan.*

*Sessão 54 — Julho 2026 — **Onboarding: "O que mais te incomoda" substituiu "O que você mais quer para sua pele", e o campo `objetivo` foi removido de todas as IAs.** A pedido do mentor: a tela `goal.tsx` ("Objetivo principal", que gravava `onboarding.objetivo`) foi **deletada** e a `concerns.tsx` ("O que mais te incomoda") subiu para o lugar dela — novo trecho do fluxo: `pregnancy* → concerns → goal-validation → skin-type` (gender/pregnancy agora navegam para `concerns`; `concerns` → `goal-validation`; `goal-validation` → `skin-type`; progresso de `concerns` = passo 3). Na última tela (`plan-preview`), a seção **"Resultados garantidos"** deixou de refletir a seleção da tela apagada e passou a mostrar **4 objetivos fixos** ("Glow natural e saudável", "Acabar com espinhas", "Pele mais firme e jovem", "Encontrar os melhores produtos"). **Decisão central:** como `objetivo` deixou de ser coletado, ele foi **removido de todas as 5 IAs e do encanamento que o alimentava** — `generate-protocol`, `analyze-skin`/`analyze-skin-app` (incl. o bloco `goal_alignment`, então dormiente — **revivido como `concerns_alignment` na Sessão 55**), `niks-chat`, `analisar-produto`, `recomendar-produtos`; e no app saiu do store (tipo/default/save), de `protocolo.tsx`, dos payloads de `loading`/`loading-dentro-app` e do tipo `UserRow`. Motivo dado pelo usuário: sem a tela, `objetivo` viraria valor fixo/genérico para todo mundo — tirar reduz ruído e deixa a IA com informação mais objetiva. **A coluna `users.objetivo` foi preservada** (sem migração) — só não é mais lida/escrita. Ver "Fluxo de telas", a tabela de Edge Functions e o tipo `ScanResult` (`concerns_alignment`).*

*Sessão 53 — Julho 2026 — **Fix da fila da "Dica do dia": 24h reais em vez de virar à meia-noite.** A fila avançava quando o **dia civil mudava** (chave `dica_do_dia:last_day`), então abrir o app às 23h50 e de novo às 00h10 já pulava para a dica seguinte — 10 min, não 24h. Agora `lib/dicas/dicaDoDia.ts` guarda o **horário da última dica** (`dica_do_dia:shown_at`, ms) e só anda **uma** casa **quando passam ≥ 24h**, regravando o horário — dentro da janela a dica é estável, e quem some dias volta na seguinte (nunca pula conteúdo, "quem não viu a 2ª não passa para a 3ª"). A parte **"individual por usuária" já funcionava** (índice por aparelho, começa na dica 1) — o exemplo "quem começou ontem está uma dica à frente de quem começou hoje" **é** o comportamento. **Migração suave:** usuária sem `shown_at` mantém o índice e só reinicia o relógio. Nada mais no app mudou (assinatura de `getDicaDoDia()` intacta). Ver "Feature: Dica do dia".*

*Sessão 52 — Julho 2026 — **Cerimônia (ritual) da Rotina migrada para a identidade nova + vídeos do welcome definitivos + fix de overflow do texto do passo.** **(1) Cerimônia** (`protocolo.tsx`): DM Serif→**Nunito** (inclusive o numeral Skia via `useFont(Nunito_800ExtraBold)`); **modo DIA recolorido** (gradientes pêssego→rosa em `dayGradients`, orb→rosa, teal→ink, coral→`#FF9D9D`); **modo NOITE mantido escuro/estrelado de propósito** (só coral→rosa e teal→ink) — detalhes na **seção 6**. **(2) Layout do passo à prova do texto da IA:** título+instrução+chip viraram `ScrollView` (`flex:1`) e o CTA "Concluir este passo" saiu de `position:absolute` para **em fluxo** logo abaixo → o texto nunca mais cola nem passa por baixo do botão (ver seção 6). **(3) Vídeos das 5 telas de welcome** substituídos pelos definitivos (`assets/welcome-1..5.mp4`); os antigos ficaram sem uso — ver "Ponto de entrada — Welcome".*

*Sessão 51 — Julho 2026 — **Onboarding + auth + telas de Results trazidos para a identidade "Novo design app NIKS".** Migração ampla, **só visual** (cores/fontes), sem tocar em lógica/copy/layout. **(1) As 5 telas de welcome** (`app/index.tsx`): Playfair→Nunito, teal→ink, coral `#FB7B6B`→rosa `#FF9D9D`; os vídeos-mockup mostram a UI ANTIGA → usuário vai enviar novos (slots `VideoClip` intactos); copy da tela 4 virou produtos. **(2) As 15 telas de PERGUNTA** (`birthday`…`social-proof`, incl. rotas condicionais) + **`scan-prep`** + **auth** (`login`, `signup`, `nome`/`components/onboarding/NameCapture.tsx`, `notifications`): transformação mecânica — trocar os VALORES dos tokens legados (`DEEP`→`#121212`, `DEEP_SOFT`→`#515151`, `CORAL`→`#FF9D9D`, `CORAL_DEEP`→`#F2808E`, `DEEP_HAIR`→`rgba(18,18,18,.10)`), Playfair Italic→Nunito ExtraBold `#FF9D9D`, `fontFamily` Nunito em todo `<Text>`. **Referência canônica: `birthday.tsx`.** **(3) `loading` do onboarding** virou o **círculo de porcentagem** copiado de `loading-dentro-app.tsx` (preservando a lógica `analyze-skin` + navegação p/ `results`, e a copy dos 7 passos como frases rotativas). Bug corrigido nas 3 telas de loading (`loading`/`loading-dentro-app`/`product-loading`): o número da % ficava descentralizado (→ espaçador `%` invisível à esquerda) e alto (descender da Nunito → `translateY:12`). **(4) `results` (onboarding) + `skin-result` (app)** ficaram idênticas na identidade nova (coral→`#FF9D9D`, teal→`#1A1A1A`, títulos Georgia serif→sans bold `#FF9D9D`, beges→neutro `#E3E3E6`/`#F4F4F4`); **verde/âmbar/vermelho semânticos mantidos**. **(5) `plan-preview`**: coral→rosa, serifs ("pronta"/"78%/68%"/"garantidos")→Nunito, gradiente do card "EM 8 SEMANAS"→rosa. ⚠️ **Só falta o paywall** (Superwall, externo). ⚠️ **Incidente:** um subagente rodou `git stash`+`drop` e apagou o trabalho paralelo de todos — recuperado via `git fsck` (dangling commit). **Regra nova: subagentes NUNCA rodam git.** Ver REGRAS (exceções de onboarding/auth) e "Ponto de entrada — Welcome".*

*Sessão 50 — Julho 2026 — **Polimento dos adesivos + bug de layout do preview.** Continuação da Sessão 49, com três decisões novas. **(1) Anel + marca do círculo seguem a MANCHETE** (`stickerColor` no `NiksSticker`): sem score, o adesivo de métrica isolada pinta o anel na cor da 1ª métrica — Oleosidade 42 amarela → **anel amarelo**, não mais o rosa fixo do score. **(2) Métrica isolada virou réplica EXATA do adesivo de score** — número gigante u(52) (era u(30)) + rótulo **completo** ("Qualidade da pele", não "Qualidade") e em negrito; o split full-vs-short de rótulo mora no `NiksStickerView` (`!solo`). **(3) Bug de layout consertado:** a pílula "Trocar adesivo" + hint não estavam no orçamento vertical (`SUBCAP_BLOCK`), então a colagem 9:16 transbordava e **cobria o header** — agora descontados de `availH`. Detalhes nas seções duráveis; ver "Feature: Compartilhar Niks score".*
