// DIRECTION — "CLÍNICA WHITE"
// Exatamente a Clínica original, mas com fundo branco puro em vez de pearl/cream.
// Isso é só uma prova de conceito — ver como a tela respira com fundo branco.

function ProtocoloClinicaWhite({ accent = PROTO.coral, displayFont = PROTO.serif, mode = 'am' }) {
  ...
  // Fundo branco puro, cards com tint sutil para criar profundidade
  const BG = '#FFFFFF';
  const CARD = '#FAF8F4'; // cards agora em creme para se destacarem do branco
  const CARD_CURRENT = '#FFFFFF';
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '28px 24px 0' }}>
        <div style={{
          display: 'flex', background: CARD, borderRadius: 100,
          padding: 4, border: '0.5px solid rgba(0,0,0,0.06)',
        }}>
          {['am', 'pm'].map(m => {
            const active = m === mode;
            return (
      ...
      <div style={{ padding: '24px 18px 0' }}>
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} style={{
              background: current ? CARD_CURRENT : CARD, borderRadius: 20, marginBottom: 10,
              padding: '18px 18px 18px 22px', position: 'relative',
              border: current ? `0.5px solid ${accent}33` : '0.5px solid rgba(0,0,0,0.05)',
              boxShadow: current ? '0 4px 24px rgba(251,123,107,0.12)' : 'none',
              ...
              <div style={{ flexShrink: 0, width: 40, textAlign: 'center' }}>
                ...
                <div style={{ height: 1, width: 20, background: accent, margin: '6px auto', opacity: 0.5 }}/>
                <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{
                  fontSize: 12, color: PROTO.inkSoft, lineHeight: 1.5, marginTop: 4,
                }}>{s.s}</div>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 6, marginTop: 10,
                  fontSize: 10, fontWeight: 500, letterSpacing: 0.5,
                  color: PROTO.gray6, fontFamily: PROTO.mono,
                }}>
...
Object.assign(window, { ProtocoloClinicaWhite });
