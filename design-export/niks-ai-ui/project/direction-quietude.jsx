// DIRECTION — "QUIETUDE"
// O equilíbrio entre leveza e substância.
// Fundo off-white suave (não branco puro, não creme).
// Um único "objeto afetivo" no topo — uma esfera orgânica com gradient sutil,
// que funciona como talismã visual (não é logo, não é ilustração).
// Passos em lista respirada, com um fio coral só no passo atual.
// Nada mais. Mas cada elemento é cuidado.

function ProtocoloQuietude({ accent = PROTO.coral, displayFont = PROTO.serif, mode = 'am' }) {
  ...
  // Off-white suave. Aquece no AM, esfria levemente no PM — mas ambos neutros.
  const bg = mode === 'am' ? '#FBF8F2' : '#F5F0E8';
  const ink = '#2B2724';
  const inkSoft = 'rgba(43,39,36,0.55)';
  const inkHair = 'rgba(43,39,36,0.10)';
  const inkWhisper = 'rgba(43,39,36,0.35)';

  // Gradient do talismã — orgânico, suave, leva o olho
  const orb = mode === 'am'
  ...
  return (
    <div style={{
      ...
      {/* Masthead — apenas duas palavras em caps minúsculo */}
      <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* specular highlight — makes it feel like glass */}
          <div style={{
      ...
      {/* ══════ TITLE BLOCK — serif generoso mas calmo ══════ */}
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontStyle: 'italic' }}>Hoje,</span> quatro gestos.
        </div>

        {/* Ornamental rule with coral dot — single point of color */}
        <div style={{
        ...
        <div style={{
          ...
          {totalMin} minutos &nbsp;·&nbsp; score 72
        </div>
      </div>

      {/* ══════ STEPS — lista respirada ══════ */}
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              {/* current indicator — the one thing that has coral */}
              {current && (
                <div style={{
              ...
              {/* Numeral romano em itálico */}
              <div style={{
                fontFamily: displayFont, fontSize: 16, fontWeight: 400,
                color: current ? accent : inkWhisper, fontStyle: 'italic',
...
Object.assign(window, { ProtocoloQuietude });
